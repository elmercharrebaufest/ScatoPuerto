using System;
using System.Linq;
using Microsoft.Activities.UnitTesting;
using Molinos.Scato.Actividades.Behaviour;
using Molinos.Scato.Actividades.Internas;
using Molinos.Scato.Dominio.Comandos;
using Molinos.Scato.Dominio.Dto;
using Molinos.Scato.Dominio.Enums;
using Molinos.Scato.Servicios;
using Moq;
using NUnit.Framework;

namespace Molinos.Scato.Test.Actividades
{
    [TestFixture]
    public class IngresarPesoTest
    {
        private IngresarPeso target;
        private Mock<IServicioComandos> srvComandos;
        private Mock<IServicioRepositorio> srvRepositorio;
        private WorkflowInvokerTest host;
        private ScatoPersistenceParticipant participant;

        [SetUp]
        public void SetUp()
        {
            target = new IngresarPeso();
            srvComandos = new Mock<IServicioComandos>();
            srvRepositorio = new Mock<IServicioRepositorio>();
            host = WorkflowInvokerTest.Create(target);
            host.Extensions.Add(srvComandos.Object);
            host.Extensions.Add(srvRepositorio.Object);
            participant = new ScatoPersistenceParticipant();
            host.Extensions.Add(participant);
        }

        [Test]
        public void TestIngresarPeso()
        {
            const int peso = 10000;
            srvComandos.Setup(s => s.Ejecutar(It.IsAny<Comando>())).Returns(new Resultado());
            srvRepositorio.Setup(s => s.ObtenerRecorridoPorGuid(It.IsAny<Guid>())).Returns(new RecorridoDto());
            host.InArguments.InstanceId = new Guid();
            host.InArguments.Peso = peso;
            host.InArguments.AlamacenId = 1;
            host.InArguments.HidraulicaId = null;
            host.InArguments.CalleId = null;
            host.InArguments.TipoPesada = TipoPesada.Bruto;

            var resultado = host.TestActivity();

            var result = resultado.First(f => f.Key == "Result").Value as Resultado;
            Assert.That(resultado, Is.Not.Null);
            Assert.That(result.HayErrores, Is.EqualTo(false));
            Assert.That(participant.DatosProximaActividad, Is.EqualTo(null));
        }

        [Test]
        public void TestErrorGenerico()
        {
            const int peso = 10000;
            srvComandos.Setup(s => s.Ejecutar(It.IsAny<Comando>())).Throws(new Exception());
            srvRepositorio.Setup(s => s.ObtenerRecorridoPorGuid(It.IsAny<Guid>())).Returns(new RecorridoDto());
            host.InArguments.InstanceId = new Guid();
            host.InArguments.Peso = peso;
            host.InArguments.AlamacenId = 1;
            host.InArguments.HidraulicaId = null;
            host.InArguments.CalleId = null;
            host.InArguments.TipoPesada = TipoPesada.Bruto;

            var resultado = host.TestActivity();

            var result = resultado.First(f => f.Key == "Result").Value as Resultado;
            Assert.That(resultado, Is.Not.Null);
            Assert.That(result.HayErrores, Is.EqualTo(true));
        }



    }
}
