...
using CrearOrdenDeDescargaFason = Molinos.Scato.Actividades.Internas.CrearOrdenDeDescargaFason;

namespace Molinos.Scato.Test.Actividades
{
    [TestFixture]
    public class CrearOrdenDeDescargaFasonTest
    {
        private CrearOrdenDeDescargaFason target;
        private OrdenDeDescargaFasonDto orden;
        ...
        public void TestIngresar()
        {
            ...
            srvComandosMock.Setup(s => s.Ejecutar(It.IsAny<Dominio.Comandos.CrearOrdenDeDescargaFason>())).Returns(new ResultadoCrear());

            var newOrden = new OrdenDeDescargaFasonDto
            {
                Chofer = orden.Chofer,
                Id = 1,
            ...
            srvRepopsitorio.Setup(s => s.ObtenerOrdenDeDescargaFason(It.IsAny<int>())).Returns(newOrden);
            ...
            var ordenResultado = resultado.First(f => f.Key == "OrdenDeDescargaFason").Value as OrdenDeDescargaFasonDto;

            Assert.That(resultado, Is.Not.Null);
            Assert.That(result.HayErrores, Is.EqualTo(false));
            Assert.That(tipoDocumento, Is.EqualTo(TipoDocumentoIngreso.OrdenDeDescargaFason));
