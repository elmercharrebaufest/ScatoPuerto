...
{
    [TestFixture]
    public class ProcesadorActualizarDescargaUnidadTest
    {
        private ProcesadorActualizarDescargaUnidad target;
        ...
        public void SetUp()
        {
            ...
            target = new ProcesadorActualizarDescargaUnidad(repositorioMock.Object, conversor, new NullLogger(), servicioSapMock.Object);
        ...
        public void TestEjecutarModificacion()
        {
            ...
            repositorioMock.Setup(s => s.Obtener(It.IsAny<Expression<Func<DescargaUnidad, bool>>>())).Returns(descargaUnidad);
            var comando = new ActualizarDescargaUnidad() { Dto = new DescargaUnidadDto() };
            var resultado = target.Ejecutar(comando);
            repositorioMock.Verify(s => s.Agregar(It.Is<DescargaUnidad>(r => r.Id == descargaUnidad.Id)), Times.Exactly(0));
        ...
        public void TestEjecutarAlta()
        {
            ...
            repositorioMock.Setup(s => s.Obtener(It.IsAny<Expression<Func<DescargaUnidad, bool>>>())).Returns((DescargaUnidad)null);
            repositorioMock.Setup(s => s.Obtener<Proveedor>(It.IsAny<Expression<Func<Proveedor, bool>>>())).Returns(new Proveedor());
            var comando = new ActualizarDescargaUnidad() { Dto = new DescargaUnidadDto() };
            var resultado = target.Ejecutar(comando);
            repositorioMock.Verify(s => s.Agregar(It.Is<DescargaUnidad>(r => r.Id == 0)), Times.Exactly(1));
            repositorioMock.Verify(s => s.GuardarCambios(), Times.Exactly(1));
            Assert.That(resultado, Is.Not.Null);
            Assert.That(resultado.HayErrores, Is.EqualTo(false));
        }
