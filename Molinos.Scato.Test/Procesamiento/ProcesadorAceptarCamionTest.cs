...
{
    [TestFixture]
    public class ProcesadorAceptarCamionTest
    {
        private ProcesadorAceptarCamion target;
        ...
        public void SetUp()
        {
            ...
            log = new NullLogger();
            target = new ProcesadorAceptarCamion(repositorioMock.Object, conversorMock.Object, log);
        ...
        public void TestModificar()
        {
            ...
            var resultado = target.Ejecutar(new AceptarCamion { WorkflowId = new Guid() });
            repositorioMock.Verify(v => v.GuardarCambios(), Times.Once());
            Assert.That(resultado.HayErrores, Is.EqualTo(false));
            Assert.True(!recorrido.Rechazado);
        }

        [Test]
        public void TestModificarError()
        {
            repositorioMock.Setup(s => s.Obtener(It.IsAny<Expression<Func<Recorrido, bool>>>())).Throws(new Exception("error"));
            var resultado = target.Ejecutar(new AceptarCamion { WorkflowId = new Guid() });
