...
{
    [TestFixture]
    public class ProcesadorModificarRecorridoEstablecimientoTest
    {
        private ProcesadorModificarRecorridoEstablecimiento target;
        ...
        public void SetUp()
        {
            ...
            log = new NullLogger();
            target = new ProcesadorModificarRecorridoEstablecimiento(repositorioMock.Object, conversorMock.Object, log);
        ...
        public void TestModificarEstablecimiento()
        {
            ...
            repositorioMock.Setup(s => s.Obtener<Establecimiento>(It.IsAny<int>())).Returns(new Establecimiento());
            var resultado = target.Ejecutar(new ModificarRecorridoEstablecimiento { InstanceId = new Guid(), EstablecimientoId = 1 });
            repositorioMock.Verify(v => v.GuardarCambios(), Times.Once());
            repositorioMock.Verify(v => v.Obtener<Establecimiento>(It.IsAny<int>()), Times.Once());
            Assert.That(resultado.HayErrores, Is.EqualTo(false));
            Assert.That(recorrido.Establecimiento, Is.Not.EqualTo(null));
        }
