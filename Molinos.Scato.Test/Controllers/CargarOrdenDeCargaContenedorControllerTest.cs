...
namespace Molinos.Scato.Test.Controllers
{
    ...
    [TestFixture]
    public class CargarOrdenDeCargaContenedorControllerTest
    {
        private CargarOrdenDeCargaContenedorController target;
        private Mock<IServicioRepositorio> servRepositorioMock;
        private Mock<IServicioComandos> servComandosMock;
        private Mock<IServicioActividadFactory<ICargarOrdenDeCargaContenedorService>> actFactoryMock;
        private Mock<ICargarOrdenDeCargaContenedorService> contractMock;
        ...
        private List<TaraContenedorDto> contenedores;
        ...
        public void SetUp()
        {
            ...
            actFactoryMock = new Mock<IServicioActividadFactory<ICargarOrdenDeCargaContenedorService>>();
            contractMock = new Mock<ICargarOrdenDeCargaContenedorService>();
            listaMock = new Mock<IListaDeWorkflows>();
            logger = new NullLogger();
            target = new CargarOrdenDeCargaContenedorController(logger, servRepositorioMock.Object, actFactoryMock.Object, servComandosMock.Object, listaMock.Object);
            ...
            dto = new OrdenDeCargaContenedorDto()
                {
                OrdenDeCargaContenedor = "77777777",
            ...
            contenedores = new List<TaraContenedorDto>
                {
                    new TaraContenedorDto {Id = 1, Descripcion = "TC1", CodigoContenedor = "AAA111", PesoTara = 12},
                    new TaraContenedorDto {Id = 2, Descripcion = "TC2", CodigoContenedor = "AAA111", PesoTara = 12}
                };
            ...
            servRepositorioMock.Setup(s => s.ObtenerNumeroOrdenDeCargaContenedorGenerado(It.IsAny<int>())).Returns("0101-00000010");
            servRepositorioMock.Setup(s => s.ListarTiposComercialesPorWfCodigo(It.IsAny<string>())).Returns(tiposComerciales);
            servRepositorioMock.Setup(s => s.ListarTaraContenedores()).Returns(contenedores);
            ...
            servRepositorioMock.Setup(s => s.ObtenerTaraContenedor(It.IsAny<int>())).Returns(new TaraContenedorDto { Id = 1, Descripcion = "Tara Contenedor" });
            actFactoryMock.Setup(s => s.CrearServicio(It.IsAny<int>())).Returns(contractMock.Object);
            contractMock.Setup(s => s.CargarOrdenDeCargaContenedor(dto, It.IsAny<int>(), workflow, It.IsAny<int>(), It.IsAny<string>(), It.IsAny<ControlRecorridoDto>())).Returns(new ResultadoCrearWorkflow { InstanciaWorkflowId = instanciaWfId });
        ...
        public void TestIndex()
        {
            ...
            var model = result.Model as OrdenDeCargaContenedorDto;
            ...
            Assert.That(model.OrdenDeCargaContenedor, Is.EqualTo(centroDto.CodigoSAP + "-" + "00000010"));
        ...
        {
            servRepositorioMock.Setup(s => s.ObtenerOrdenDeCargaContenedorPorNumeroDeOrden(It.IsAny<string>())).Returns(new OrdenDeCargaContenedorDto());
        ...
        public void TestOrdeDeCargaContenedorExistente()
        {
            ...
            servRepositorioMock.Setup(s => s.ObtenerOrdenDeCargaContenedorPorNumeroDeOrden(It.IsAny<string>()))
               .Returns(new OrdenDeCargaContenedorDto { Id = 1});

            var result = target.Index(workflow, dto, new DatosUsuario { CentroId = 1 }) as ViewResult;
            ValidarVistaConError(result);
            Assert.That(target.ModelState.First().Value.Errors.First().ErrorMessage, Is.EqualTo(Textos.OrdenDeCargaContenedor_Existente));
        }
        ...
        public void TestChoferEnOtroRecorrido()
        {
            ...
            var result = target.Index(workflow, dto, new DatosUsuario { CentroId = 1 }) as ViewResult;
            ValidarVistaConError(result);
            Assert.That(target.ModelState.First().Value.Errors.First().ErrorMessage, Is.EqualTo(string.Format(Textos.Error_ChoferYaEstaEnPlanta, dto.Chofer.NombreCompleto, dto.OrdenDeCargaContenedor, dto.PatenteCamion)));
        }
        ...
        private void ValidarVistaConError(ViewResult result)
        {
            ...
            IEnumerable<SelectListItem> tiposComercialesVb = target.ViewBag.TiposComerciales;
            IEnumerable<SelectListItem> tiposDocumentosVb = target.ViewBag.TiposDocumentos;
            Assert.That(model.OrdenDeCargaContenedor, Is.EqualTo("77777777"));
            Assert.That(tiposComercialesVb.Select(s => s.Text), Is.EquivalentTo(new List<string> { "D1", "D2" }));
            Assert.That(tiposDocumentosVb.Select(s => s.Text), Is.EquivalentTo(new List<string> { "T1" }));
            Assert.That((string)target.ViewBag.Workflow, Is.EqualTo("W1"));
