...
namespace Molinos.Scato.Test.Controllers
{
    ...
    [TestFixture]
    public class IngresoRemitoTercerosControllerTest
    {
        private IngresoRemitoTercerosController target;
        private Mock<IServicioRepositorio> servRepositorioMock;
        private Mock<IServicioComandos> servComandosMock;
        private Mock<IServicioActividadFactory<IIngresoRemitoTercerosService>> actFactoryMock;
        private Mock<IIngresoRemitoTercerosService> contractMock;
        ...
        public void SetUp()
        {
            ...
            actFactoryMock = new Mock<IServicioActividadFactory<IIngresoRemitoTercerosService>>();
            contractMock = new Mock<IIngresoRemitoTercerosService>();
            listaMock = new Mock<IListaDeWorkflows>();
            logger = new NullLogger();
            target = new IngresoRemitoTercerosController(logger, servRepositorioMock.Object, actFactoryMock.Object, servComandosMock.Object, listaMock.Object);
            ...
            servRepositorioMock.Setup(s => s.ObtenerProveedor(It.IsAny<int>())).Returns(new ProveedorDto());
            servRepositorioMock.Setup(s => s.ObtenerNumeroRemitoGenerado()).Returns(10);
            ...
            servRepositorioMock.Setup(s => s.ObtenerNumeroOrdenDeDescargaGenerado(It.IsAny<int>())).Returns("0101-00000000");

            contractMock.Setup(s => s.IngresoRemitoTerceros(dto, It.IsAny<int>(), workflow, It.IsAny<int>(), It.IsAny<string>(), It.IsAny<ControlRecorridoDto>())).Returns(new ResultadoCrearWorkflow { InstanciaWorkflowId = instanciaWfId });
        ...
        public void TestIndex()
        {
            ...
            var model = result.Model as RemitoDto;
            ...
            Assert.That(model.OrdenDeDescarga, Is.EqualTo(centroDto.CodigoSAP + "-" + "00000000"));
            Assert.That(model.FechaOD.Day, Is.EqualTo(DateTime.Now.Day));
            Assert.That(model.EsRemitoProveedor, Is.EqualTo(true));
        ...
        {
            servRepositorioMock.Setup(s => s.ObtenerRemitoPorNroRemito(It.IsAny<string>())).Returns(new RemitoDto());
        ...
        private void ValidarVistaConError(ViewResult result)
        {
            ...
            IEnumerable<SelectListItem> tiposComercialesVb = target.ViewBag.TiposComerciales;
            IEnumerable<SelectListItem> tiposDocumentosVb = target.ViewBag.TiposDocumentos;
            Assert.That(model.OrdenDeDescarga, Is.EqualTo("77777777"));
            Assert.That(model.FechaOD.Day, Is.EqualTo(DateTime.Now.Day));
            Assert.That(tiposComercialesVb.Select(s => s.Text), Is.EquivalentTo(new List<string> { "D1", "D2" }));
            Assert.That(tiposDocumentosVb.Select(s => s.Text), Is.EquivalentTo(new List<string> { "T1" }));
            Assert.That((string)target.ViewBag.Workflow, Is.EqualTo("W1"));
        ...
        public void TestChoferEnOtroRecorrido()
        {
            ...
            var result = target.Index(workflow, dto, new DatosUsuario { CentroId = 1 }) as ViewResult;
            ValidarVistaConError(result);
            Assert.That(target.ModelState.First().Value.Errors.First().ErrorMessage, Is.EqualTo(string.Format(Textos.Error_ChoferYaEstaEnPlanta, dto.Chofer.NombreCompleto, dto.OrdenDeDescarga, dto.PatenteCamion)));
        }
    }
}
