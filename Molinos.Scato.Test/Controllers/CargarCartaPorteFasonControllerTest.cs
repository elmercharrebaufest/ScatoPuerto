...
namespace Molinos.Scato.Test.Controllers
{
    ...
    [TestFixture]
    public class CargarCartaPorteFasonControllerTest
    {
        private CargarCartaPorteFasonController target;
        private Mock<IServicioRepositorio> servRepositorioMock;
        private Mock<IServicioComandos> servComandosMock;
        private Mock<IServicioActividadFactory<ICargarCartaPorteFasonService>> actFactoryMock;
        private Mock<ICargarCartaPorteFasonService> contractMock;
        ...
        public void SetUp()
        {
            ...
            actFactoryMock = new Mock<IServicioActividadFactory<ICargarCartaPorteFasonService>>();
            contractMock = new Mock<ICargarCartaPorteFasonService>();
            listaMock = new Mock<IListaDeWorkflows>();
            configuracionMock = new Mock<IFirmaProvider>();
            logger = new NullLogger();
            target = new CargarCartaPorteFasonController(logger, servRepositorioMock.Object, actFactoryMock.Object, servComandosMock.Object, listaMock.Object, configuracionMock.Object);
            ...
            servRepositorioMock.Setup(s => s.NumeroCartaPorteValido(It.IsAny<string>(), It.IsAny<int>(), It.IsAny<string>())).Returns(new CartaPorteValidaResponseDto{Valida = true});
            ...
            contractMock.Setup(s => s.CargarCartaPorteFason(It.IsAny<CartaPorteDto>(), It.IsAny<VehiculoDto>(), It.IsAny<int>(), workflow, It.IsAny<int>(), It.IsAny<string>(), It.IsAny<ControlRecorridoDto>())).Returns(new ResultadoCrearWorkflow { InstanciaWorkflowId = InstanciaWfId });
            servRepositorioMock.Setup(s => s.ObtenerCartaPorteVaciaFason(It.IsAny<int>(), It.IsAny<string>(), It.IsAny<string>(), It.IsAny<string>())).Returns(new CartaPorteDto());
            configuracionMock.Setup(s => s.ObtenerFirmaSinLogo()).Returns(new FirmaDto { CodigoSAP = "9950085862" });
            servRepositorioMock.Setup(s => s.ListarCategorias()).Returns(new List<CategoriaDto>());
        ...
        public void TestChoferEnOtroRecorrido()
        {
            ...
            var result = target.Index(workflow, dto, new DatosUsuario { CentroId = 1 }) as ViewResult;
            ValidarVistaConError(result);
            Assert.That(target.ModelState.First().Value.Errors.First().ErrorMessage, Is.EqualTo(string.Format(Textos.Error_ChoferYaEstaEnPlanta, dto.Chofer.NombreCompleto, dto.NroCartaPorte, "aaa111")));
