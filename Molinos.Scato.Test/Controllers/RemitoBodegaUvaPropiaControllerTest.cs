...
namespace Molinos.Scato.Test.Controllers
{
    ...
    [TestFixture]
    public class RemitoBodegaUvaPropiaControllerTest
    {
        private RemitoBodegaUvaPropiaController target;
        ...
        public void SetUp()
        {
            ...
            target = new RemitoBodegaUvaPropiaController(logger, servRepositorioMock.Object, actFactoryMock.Object, servComandosMock.Object, listaMock.Object, configuracionMock.Object);
            ...
            servRepositorioMock.Setup(s => s.ListarVinedosPropios()).Returns(new List<VinedoPropioDto> { new VinedoPropioDto() { Id = 1, Descripcion = "Vinedo 1" } });
        ...
        [Test]
        public void FiltrarCuartelesPorVinedoTest()
        {
            servRepositorioMock.Setup(s => s.FiltrarCuartelesPorVinedo(It.IsAny<int>())).Returns(new List<CuartelDto> {new CuartelDto {Id = 1, Codigo = "1"}});

            var resultado = target.FiltrarCuartelesPorVinedo(1) as JsonResult;
            var serializer = new JavaScriptSerializer();
            var output = serializer.Serialize(resultado.Data);
            Assert.That(output, Is.EqualTo("[{\"Selected\":false,\"Text\":\"1\",\"Value\":\"1\"}]"));
        }

        [Test]
        public void ObtenerDescargasTest()
        {
            servRepositorioMock.Setup(s => s.ObtenerDescargasDeBinesPorRemitoBodegaUva(It.IsAny<int>())).Returns(new List<DescargaDeBinesDto> { new DescargaDeBinesDto() { Id = 1, CantidadBines = 1,} });

            var resultado = target.ObtenerDescargas(1) as JsonResult;
            var serializer = new JavaScriptSerializer();
            var output = serializer.Serialize(resultado.Data);
            Assert.That(output, Is.EqualTo("[{\"Id\":1,\"TipoId\":0,\"Tipo\":null,\"CuartelId\":0,\"Cuartel\":null,\"CantidadBines\":1,\"RemitoBodegaUvaId\":0,\"Peso\":0,\"EsGranel\":null}]"));
        }

        [Test]
        public void ObtenerClasePorMaterialTest()
        {
            servRepositorioMock.Setup(s => s.ObtenerMaterial(It.IsAny<int>())).Returns(new MaterialDto { Id = 1, Clase = ClaseBin.Bines });

            var resultado = target.ObtenerClasePorMaterial(1) as JsonResult;
            var serializer = new JavaScriptSerializer();
            var output = serializer.Serialize(resultado.Data);
            Assert.That(output, Is.EqualTo("{\"resultado\":0}"));
        }
    }
}
