...
namespace Molinos.Scato.Test.Controllers
{
    ...
    public class ConsultasControllerTest
    {
        ...
        {
            servRepositorioMock.Setup(s => s.BuscarMaterialesPorCentro(It.IsAny<int>(),It.IsAny<string>())).Returns(new List<MaterialPorCentroDto>
                {
                    new MaterialPorCentroDto() { Id = 1, MaterialDesc = "Mat"}
                });

            var result = target.BuscarMateriales("term", new DatosUsuario{CentroId = 1}) as JsonResult;
            Assert.NotNull(result);
            var resultado = serializer.Deserialize<List<JsonObjeto>>(serializer.Serialize(result.Data));
            Assert.AreEqual(resultado.Count, 1);
