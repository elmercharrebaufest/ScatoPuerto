...
namespace Molinos.Scato.Web.Helpers
{
    public static class ExtensionesWebGrid
    {
        ...
        {
            var pendiente = PermisosScato.CamionesPendientesMesa.DisplayText();
            return grid.Column("ejecutar", "", f => html.Raw(html.BotonLink(Textos.Ejecutar, ((string)f.ProximaAccion) == pendiente ? "EjecutarPendiente" : "Ejecutar", "ListaDeCamiones", new { Id = ((string)f.ProximaAccion) == pendiente ? f.RecorridoId : f.id, proximaAccion = (string)f.ProximaAccion, codigo = f.Codigo }, style + (f.Reingreso ? " rowReingresado" : "") + (f.Rechazado ? " rowRechazado " : ""), "icon-play", true).ToHtmlString()), "editar-borrar-columna", false);
        }
        ...
        {
            var pendiente = PermisosScato.CamionesPendientesMesa.DisplayText();
            return grid.Column("Patente", "Patente", f => ((string)f.ProximaAccion) == pendiente ? html.Raw((String)f.Patente) : html.Raw(html.BotonLink((String)f.Patente, "Seleccionar", controller, new { Id = (int)f.RecorridoId, actualizarCookie = true }, "ajax-popup-link2", "", false, false).ToHtmlString()),
                                                        "", true);
        }
