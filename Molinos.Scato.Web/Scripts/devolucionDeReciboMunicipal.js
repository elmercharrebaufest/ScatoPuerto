jQuery(document).ready(function ($) {
    ...
    $(".btn").prop('disabled', true);
    $("#btnCobrarEnEfectivo").click(avanzarDeEtapa);
    $("#btnDevolverPago").click(devolverPago);

    $("#btnCancelar").click(limpiarVentana);
    ...
    $.ajax({
        ...
        success: function (data) {
            if (data.HayErrores === true) {
                limpiarVentana();
                mostrarAlertaPorPantalla(2, data.Error.Value === undefined ? data.Error : data.Error.Value);
            } else {
                ...
                if (data.Devuelto === true) {
                    ...
                    cambiarMensajeDeEstado("esperandoDevolucion");
                }
...
function devolverPago() {

    $.ajax({
        url: $("#links").data().urlDevolucion,
        dataType: 'json',
        data: {
            NumeroDeTarjeta: $("#numTarjeta").val(),
            RecorridoId: $("#RecorridoId").val(),
        ...
        success: function (data) {
            ...
            if (data.HayErrores) {
                ...
                let mensajeDeError = "";
                todosLosErrores.forEach(function (e) { mensajeDeError = mensajeDeError + " \n " + e });
                mostrarAlertaPorPantalla(2, "\n Error al procesar la devolución del pago: " + mensajeDeError);
            } else {
                mostrarAlertaPorPantalla(3, "Cobro reembolsado.");
                cambiarMensajeDeEstado("esperandoQr");
            }
        }
    }).fail(function () {
...
function cambiarMensajeDeEstado(estadoDelPago) {
    ...
    switch (estadoDelPago) {
        case "esperandoQr":
            ...
            $("#btnCobrarEnEfectivo").css('visibility', 'visible');
            $("#btnDevolverPago").css('visibility', 'hidden');
        ...
        case "esperandoDevolucion":
            ...
            $("#spinnerEsperandoLecturaQr").css('visibility', 'hidden');
            $(".btn").prop('disabled', false);
            $("#btnCobrarEnEfectivo").css('visibility', 'hidden');
            $("#btnDevolverPago").css('visibility', 'visible');
