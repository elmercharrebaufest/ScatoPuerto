...
jQuery(document).ready(function ($) {
    ...
    var arr = ["PesoBrutoOrigen", "PesoTaraOrigen", "Patente", "PatenteAcoplado", "KmRecorrer", "TarifaReferencia", "TarifaTonelada"];
...
/* Item Model
/*----------------------------------------------------------------------*/
function Camion(id, patente, patenteacoplado, bruto, tara, neto) {
    var self = this;
    if (bruto == 0) {
        bruto = null;
    }
...
function CamionViewModel() {
    ...
    if (camionJson != "[]" && camionJson != "" && camionJson != null) {
        ...
        self.camion.push(new Camion(camionAux[0].Id, camionAux[0].Patente, camionAux[0].PatenteAcoplado, camionAux[0].PesoBrutoOrigen, camionAux[0].PesoTaraOrigen, camionAux[0].PesoNetoOrigen));
    } else {
...
function ActualizarTipoVehiculo(patente, acoplado, before, callback) {    
    if ((CorrespondeNotificarPorNetoMaximo()) && (patente != "" || acoplado != "")) {
            ...
            if (data.CodigoDeError == 0) {
                if (data.Categoria != null) {
                    if ($('#tipoVehiculoDropdown option[value=' + data.Categoria + ']').length == 0) {
                        if ((CorrespondeNotificarPorNetoMaximo())) MostrarAlertaError("La categoría del vehículo " + data.CategoriaDesc + " no esta configurada para el centro actual");
                    } else {
                    ...
                    if ((CorrespondeNotificarPorNetoMaximo())) MostrarAlertaError("El servicio CNRT no devolvió información sobre la categoría del vehículo, debe ingresarla manualmente.");
                }
