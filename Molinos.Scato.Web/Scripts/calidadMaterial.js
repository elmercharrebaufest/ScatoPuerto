jQuery(document).ready(function ($) {
    var elementoId = "#materialIdFiltro";
    
    DefinirAutocompletar('#materialFiltro', elementoId, $('#links').data().urlBuscarMateriales, $('#links').data().urlBuscarMaterial, CargarCalidades);
    
    $(document).on('click', '.ajax-editar-custom-link', function () {
        $.get(this.href + "?materialId=" + $(elementoId).val(), cargarDialogoEditar2);
        return false;
    });
});

function CargarCalidades(material) {
    $.get($('#ConsultarCalidadesMaterial').val(), { materialId: material || $("#materialIdFiltro").val() }, function(data) {
...
function editarRepuestaFormulario2(respuesta) {
    if (respuesta == window.ajaxEditSuccess) {
        if ($('#search-form').length == 0) {
            ...
            CargarCalidades($("#MaterialId").val());
