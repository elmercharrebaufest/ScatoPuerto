...
{
    public class PdfLoteReporte : ActionResult
    {
        private readonly string fileName;
        private readonly LoteDto loteDto;
        private readonly IFirmaProvider firma;

        public PdfLoteReporte(string fileName, LoteDto loteDto, IFirmaProvider firma)
        {
        ...
        private void DefineTables(Document document)
        {
            ...
            row.Cells[0].AddParagraph(Textos.Producto);
            row.Cells[1].AddParagraph(Textos.Lote_Vendedor);
            row.Cells[2].AddParagraph(Textos.CartaPorte_Corredor);
            row.Cells[3].AddParagraph(Textos.Lote_Descarga);
            row.Cells[4].AddParagraph(Textos.Lote_Kilos);
            row.Cells[5].AddParagraph(Textos.Lote_Muestra);
            row.Cells[6].AddParagraph(Textos.Centro_Localidad);
            row.Cells[7].AddParagraph(string.Empty);
            row.Cells[8].AddParagraph(Textos.Camion_Patente);
            ...
            {
                row = table.AddRow();
                AddTextToCell(muestra.Material, row.Cells[0], document);
                AddTextToCell(muestra.Vendedor, row.Cells[1], document);
                AddTextToCell(muestra.Corredor, row.Cells[2], document);
                AddTextToCell(muestra.FechaDescarga.HasValue ? muestra.FechaDescarga.Value.Formatted() : string.Empty, row.Cells[3], document);
                AddTextToCell(muestra.PesoNeto.ToString(), row.Cells[4], document);
                AddTextToCell(muestra.NroMuestra, row.Cells[5], document);
                AddTextToCell(muestra.Localidad, row.Cells[6], document);
                AddTextToCell(muestra.Patente, row.Cells[8], document);
            }
        ...
        private void DefineContentSection(Document document)
        {
            ...
            paragraph = frame.AddParagraph(string.Format(Textos.Lote_TituloReporte, loteDto.NumeroDeLote));
