...
{
    [Autorizacion(PermisosScato.ActividadCargarOrdenEntrePlantas)]
    public class CargarOrdenEntrePlantasController : DocumentoIngresoController
    {
        private readonly IServicioActividadFactory<ICargarOrdenEntrePlantasService> factory;
        private readonly IListaDeWorkflows workflows;

        public CargarOrdenEntrePlantasController(ILogger log, IServicioRepositorio servicio, IServicioActividadFactory<ICargarOrdenEntrePlantasService> factory, IServicioComandos servicioComandos, IListaDeWorkflows workflows)
        ...
        public ActionResult Index(string workflow, DatosUsuario datosUsuario)
        {
            ...
            var numero = servicio.ObtenerNumeroOrdenEntrePlantasGenerado(datosUsuario.CentroId);
            SetearVista(workflowObj);
            return View(new OrdenEntrePlantasDto { Numero = numero, Fecha = DateTime.Today });
        }

        [HttpPost]
        [DatosUsuario]
        public ActionResult Index(string workflow, OrdenEntrePlantasDto orden, DatosUsuario datosUsuario)
        {
            var workflowObj = servicio.ObtenerWorkflowPorCodigo(workflow);
            ...
            var numeroDeOrden = servicio.ObtenerOrdenEntrePlantasPorNumeroDeOrden(orden.Numero);
            if (numeroDeOrden != null)
            {
                ModelState.AddModelError("OrdenEntrePlantas", Textos.OrdenEntrePlantas_Existente);
                SetearVista(workflowObj);
                return View(orden);
            }
            ...
            if (ModelState.IsValid)
            {
                ...
                {
                    Actividad = Textos.ActCargarOrdenEntrePlantas,
                    ActividadXaml = "CargarOrdenEntrePlantas",
                    PuestoDeTrabajoId = datosUsuario.PuestoDeTrabajoId,
                    NombreUsuario = datosUsuario.NombreUsuario
                };

                var resultadoActividad = servicioWf.CargarOrdenEntrePlantas(orden, datosUsuario.CentroId, workflow, workflowDefinicionId, datosUsuario.NombreUsuario, controlRecorrido) as ResultadoCrearWorkflow;
