...
namespace Molinos.Scato.Web.Controllers
{
    ...
    public class PesadaController : BaseController
    {
        ...
        public virtual ActionResult Index(Guid id, DatosUsuario datosUsuario)
        {
            ...
            var balanzaValida = BalanzaValida(recorrido);

            if (!balanzaValida)
            {
                TempData["Alerta"] = string.Format(Textos.Pesada_BalanzaAsignadaNoDisponibleParaUsuario, recorrido.Patente);
        ...
        {
            ViewBag.Balanzas = servicio.ListarBalanzasPorNombrePc(datosUsuario.CentroId, datosUsuario.NombrePc, recorrido.TipoVehiculo).OrderBy(o => o.Nombre).ToSelectList(f => f.Id.ToString(CultureInfo.InvariantCulture), f => f.Nombre);
            ...
            var puestosdetrabajo = servicio.ListarPuestosDeTrabajoPorNombrePc(datosUsuario.NombrePc, datosUsuario.CentroId);
            if (puestosdetrabajo!=null && puestosdetrabajo.Count>0)
            {
                ViewBag.Automatizado = puestosdetrabajo.First().Automatizado;
            }
            else
            {
