...
namespace Molinos.Scato.Web.Controllers
{
    ...
    public class PuestoComandoController : BaseController
    {
        ...
        public PuestoComandoController(ILogger log, IListaDeWorkflows workflows, IServicioRepositorio servicio, IServicioComandos servicioComandos, IServicioActividadFactory<IPuestoComandoService> factory, IServicioActividadFactory<IEjecutarService> factoryejecutar)
            : base(servicio)
        {
            this.log = log;
                ...
                foreach (var camion in instanceIds.Split(',').Select(x => new Guid(x)))
                {
                    if (workflows.VerificarExistenciaDeWorkflowPorGuid(camion))
                    {
                        ...
                        else
                        {
                            ...
                            log.Error("Falló el rechazo del camión ya que no se encuentra en Puesto Comando ni en Playa Externa");
                        }
                    }
                }
                if (patentesInvalidas.Any())
                {
                    ViewBag.Error = "No es posible rechazar las siguientes patentes porque no se encuentran en Puesto Comando ni en Playa Externa:";
                    ViewBag.PatentesInvalidas = patentesInvalidas;
                }
