...
export class NominacionRecibosComponent implements OnInit {
      ...
      if (data!=null && data.actualizar){
        this.nominacionDatoTecnicoRegistroService.listarExportadores().pipe(takeUntil(this.destroy$)).subscribe((exportadores: Exportador[]) => {
          ...
          this.exportadores = this.exportadores.filter((expoData: Exportador) => {
            return data.listaExportadores.some((expoFilter: Exportador) => {
      ...
      if (parametro != null) {
        ...
        if (nominacionParametos.actualizarDatoTecnico) {
          if (nominacionParametos.nominacion != null)
            this.inicializarFormEdicion(nominacionParametos.nominacion.nominacionRecibo);
        }
      }
    });
  }
  ...
  cargarListasRecibo() {
    ...
    this.nominacionDatoTecnicoRegistroService.listarExportadores().pipe(takeUntil(this.destroy$)).subscribe((data: Exportador[]) => {
      this.exportadores = data;
      this.formatos = this.nominacionDatoTecnicoRegistroService.listarFormatos();
