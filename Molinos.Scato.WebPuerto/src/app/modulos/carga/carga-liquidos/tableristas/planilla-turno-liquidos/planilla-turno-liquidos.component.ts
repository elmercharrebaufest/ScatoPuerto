...
*/
              ...
              this.setDia(null, turnoNuevo, fechaSeleccionada)  
                    ...
                    /*
                      let planillaTurno = {
                        ...
                        moduloDeCargaPlanillaDeTurnosDetalles: [],
  ...
  fillPlanilla(){
    this.planillaDeTurnos = this.procesoService.getModuloDeCarga()?.moduloDeCargaPlanillaDeTurnos;
    ...
    if(this.planillaDeTurnos != undefined && this.planillaDeTurnos.length > 0){
      ...
      this.planillaDeTurnos.forEach((dia, indexDia)=>{
        ...
        //Agrego detalles
        if(dia.moduloDeCargaPlanillaDeTurnosDetalles?.length > 0){
          this.initTurnoDetalle(dia.moduloDeCargaPlanillaDeTurnosDetalles,
  ...
  getTurnoDetalles(d, t): FormArray {
    return this.getTurnos(d)['controls'][t]['controls'].moduloDeCargaPlanillaDeTurnosDetalles as FormArray;
  }
  ...
  getRowSpanTurno(turno: any) {
    if (turno.controls.moduloDeCargaPlanillaDeTurnosCortes.length > 0)
      return turno.controls['moduloDeCargaPlanillaDeTurnosDetalles'].controls.length + 1;
    else
      return turno.controls['moduloDeCargaPlanillaDeTurnosDetalles'].controls.length;
  }
  ...
  async enviarTurno(dia, turno) {
   // this.enviarMail();
    ...
    turnoAEnviar['moduloDeCargaPlanillaDeTurnos'].moduloDeCargaPlanillaDeTurnosDetalles = turnoAEnviar['moduloDeCargaPlanillaDeTurnos'].moduloDeCargaPlanillaDeTurnosDetalles.filter(m =>
      m.exportador ||
      m.linea ||
      m.bodegaParcel ||
      m.materialPuerto ||
    ...
    if (turnoAEnviar['moduloDeCargaPlanillaDeTurnos'].moduloDeCargaPlanillaDeTurnosDetalles.length > 0) {
  ...
  getCantTurno(t) {
    let contador = 0;
    // return 0;
    for (let turno of t['controls']['moduloDeCargaPlanillaDeTurnosDetalles'].controls) {
  ...
  initTurnoDetalle(detalle:Turno [], turno: PlanillaDeTurnos, turnoIndex?: number){
    let planillaTurnoDetalles = turno['controls'][turnoIndex]['controls']['moduloDeCargaPlanillaDeTurnosDetalles'];
     ...
     if (detalle != null){
        if(detalle.length > 0){
          ...
          for(let i=detalle.length; i<4; i++){
            (turno['controls'][turnoIndex]['controls']['moduloDeCargaPlanillaDeTurnosDetalles'] as FormArray).push(this.initLinea(null,turno['controls'][0]['controls'].cerrado.value));
          }
        }
    //Si no hay detalles completo con 4 lineas vacías.
    }else{
      for (let i= 1; i <= 4; i++) {
        (turno['controls'][turnoIndex]['controls']['moduloDeCargaPlanillaDeTurnosDetalles'] as FormArray).push(this.initLinea());
      }
    }
  }
  ...
  initTurno(turnoPuerto?: PlanillaDeTurnos, esNuevoTurno?: boolean) {
    ...
    if(!esNuevoTurno){
      fg = this._builder.group({      
        moduloDeCargaPlanillaDeTurnosDetalles:this._builder.array([]),
  ...
  getToneladasParcelDia(bodega: number, d: number) {
    ...
    for (let turno of dia.controls) {
      for (let linea of turno['controls']['moduloDeCargaPlanillaDeTurnosDetalles']['controls']) {
  ...
  getToneladasLinea(value: string) {    
    ...
    this.diasTurno.controls.forEach(dia => {
      ...
      turnos.forEach(turno => {
        const lineaTurnos = turno['controls']['moduloDeCargaPlanillaDeTurnosDetalles']['controls'];
  ...
  async generarExcelPorParcel(){
    ...
    // datosModal.parcelSeleccionados.forEach((element, i) => {
      ...
      this.planillaDeTurnos.forEach((turno:PlanillaDeTurnos) => {
        ...
        if (turno.moduloDeCargaPlanillaDeTurnosCortes.length == 0 && turno.moduloDeCargaPlanillaDeTurnosDetalles.length == 0){
            totalNumeroTurnos-=1;
        }
      });
      ...
      this.planillaDeTurnos.forEach((turno:PlanillaDeTurnos) => {
        ...
        numeroTurno += 1;
        if (turno.moduloDeCargaPlanillaDeTurnosDetalles.length > 0){
           /* Planilla de turnos */
          ...
          let registrosTurno = turno.moduloDeCargaPlanillaDeTurnosDetalles.length - 1;
          const numRegistroTurno = turno.moduloDeCargaPlanillaDeTurnosDetalles.length;
          const nombreTurno = turno.turnoPuerto.nombre;

          if (turno.moduloDeCargaPlanillaDeTurnosCortes.length > 0){
              registrosTurno = turno.moduloDeCargaPlanillaDeTurnosDetalles.length
          ...
          turno.moduloDeCargaPlanillaDeTurnosDetalles.forEach((turno: any, index) => {
        ...
        let fechaDia;
          this.planillaDeTurnos.forEach((turno: PlanillaDeTurnos) => {
            ...
            if(turno.indexDia == dia){
              ...
              if (turno.moduloDeCargaPlanillaDeTurnosDetalles.length > 0){
                CantRows = CantRows + (turno.moduloDeCargaPlanillaDeTurnosDetalles?.length + 1);
              }
              if (turno.moduloDeCargaPlanillaDeTurnosCortes.length > 0){
  ...
  calcularRestaEmbarcar(): number{
   ...
   enviarPlanillaTurno(dia, turno, mail) {
    ...
    ModuloDeCargaPlanillaDeTurnos.moduloDeCargaPlanillaDeTurnosDetalles= ModuloDeCargaPlanillaDeTurnos.moduloDeCargaPlanillaDeTurnosDetalles.filter(m =>
      m.exportador ||
      m.linea ||
      m.bodegaParcel ||
      m.materialPuerto ||
      ...
      ModuloDeCargaPlanillaDeTurnos.fecha =  this.getTurnos(dia)['controls'][turno]['controls'].turnoPuerto.value.fecha
        ...
        if (ModuloDeCargaPlanillaDeTurnos.moduloDeCargaPlanillaDeTurnosDetalles.length > 0) {
   ...
   guardarTurnoGeneral(dia, turno){
    ...
    try {
      ...
      let moduloDeCargaPlanillaDeTurnosDetalles = [];
      ...
      for (const index in Turno.moduloDeCargaPlanillaDeTurnosDetalles['controls']) {
           const turnoDetalle = Turno.moduloDeCargaPlanillaDeTurnosDetalles['controls'][index];
           ...
           if ( turnoDestinoVal != '0' && lineaIdVal != '' &&
                ...
                materialPuertoVal != '')  
              {
                 ...
                 moduloDeCargaPlanillaDeTurnosDetalles.push(objTurnosDetalles);
              }
      }
      ...
      let planillaTurno = {
        ...
        moduloDeCargaPlanillaDeTurnosDetalles: moduloDeCargaPlanillaDeTurnosDetalles,
        turnoPuerto: Turno.turnoPuerto['value'].turnoPuerto
      }

      
      if (moduloDeCargaPlanillaDeTurnosCortes.length == 0 && moduloDeCargaPlanillaDeTurnosDetalles.length == 0){
