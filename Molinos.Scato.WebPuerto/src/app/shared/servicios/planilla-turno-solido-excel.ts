...
export class PlanillaTurnoSolidoExcelService {
    ...
    private setCabeceraPlanillaTurno(worksheet, headerPlanilla, offset, borders){
    ...
    private setDetallePlanillaTurno(turno, worksheet, offset, borders) {
      ...
      for (let indexCell = 1; indexCell <= 6; indexCell++) {
        worksheet.getRow(offset).getCell(celdaDetalle).alignment = { vertical: 'middle', horizontal: 'center',  wrapText: true}
    ...
    private getNombreArchivo(){
        ...
        nombreArchivo='Planilla_Recibidores_Sol_' + horaActualFormat;
    ...
    private setCabeceraPlanillaCortes(headerCortes, worksheet, offset, borders){
            ...
            worksheet.mergeCells(`G${offset}:G${offset}`);
    ...
    async generarExcelPorParcel(procesoService, planillaDeTurnosSinFiltrar, esEnviarPlanilla: boolean = false, totalABordo: number = 0, verObservacionesCalidad: boolean = true) {
      const planillaDeTurnos = planillaDeTurnosSinFiltrar.filter(x=> x.guardadoPorRecibidor == true && x.guardadoPorTablerista == true);
        ...
        const headerPlanilla = ["Exportador", "Bodega", "Producto", "Destino", "Cant."];
        ...
        planillaDeTurnos.forEach((turno: PlanillaDeTurnos) => {
          ...
          if (turno.moduloDeCargaPlanillaDeTurnosDetallesSolido.length == 0)
            this.setAgrupadorTurnos(turno, worksheet, offset, numeroTurno, totalNumeroTurnos, borders, null, verObservacionesCalidad);
    ...
    private obtenerFechaTurno(fechaDia: Date){
