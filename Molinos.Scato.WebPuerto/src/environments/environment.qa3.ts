export const environment = {
  production: true,
  envName: 'qa3',
  apiUrl: '/Scato.WebPuertoApi/api/',
  apiAzureUrl: '/Scato.AzureAD/',
  apiGraph: 'https://graph.microsoft.com/v1.0/',
  clientIdMoa: 'ac5390bd-7225-4fe0-be95-9d2e5aa582a5',
  clientIdMoc: '5ab4c0b8-bf3b-42ef-90b9-697559762d72',
  redirectUri: 'https://scatopuertoqa3.molinosagro.com.ar/Scato.WebPuerto',
  postLogoutRedirectUri: 'https://scatopuertoqa3.molinosagro.com.ar/Scato.WebPuerto/login',
  authorityMoa: 'https://login.microsoftonline.com/790c9737-0b8e-4138-a0f4-819cdc1eb64b',
  authorityMoc: 'https://login.microsoftonline.com/c5d83817-b680-4929-9c3c-407e37ea2678',
  webPuertoApiUsername:'WebPuertoApi',
  webPuertoApiPassword:'ipidsadkjdW3QB7B'
};


