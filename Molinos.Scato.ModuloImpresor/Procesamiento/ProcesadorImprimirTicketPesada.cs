...
{
    public class ProcesadorImprimirTicketPesada : ProcesadorComando<ImprimirTicketPesada>
    {


        public ProcesadorImprimirTicketPesada(ILogger log)
            : base(log)
        {

        }

        public override Resultado Ejecutar(ImprimirTicketPesada comando)
        {
            try
            {
                ...
                for (var copia = 0; copia < comando.CantidadCopias; copia++)
                {
                    ...
                    var impresora = new TicketPesada(comando.Dto, comando.Dto.Impresora, comando.Firma);
                    impresora.Print();
                }
            }
            catch (Exception e)
