...
//using System.Data.Common.CommandTrees.ExpressionBuilder; // Comentado temporalmente para evitar conflicto con System.Linq.Expressions
    ...
    public class ServicioRepositorio : IServicioRepositorio
    {
        ...
        public BalanzadasCompletasDto BalanzadasBuque(int IdModuloDeCarga)
        {
            try
            {
                ...
                var lineup = repositorio.Obtener<LineUp>(x => x.ModuloDeCarga.Id == IdModuloDeCarga);
                if (lineup != null)
                {
        ...
        {
            var turnos = repositorio.Listar<ModuloDeCargaPlanillaDeTurnos>(x => x.ModuloDeCarga.Id == idModuloDeCarga && x.Cerrado == true).ToList();
            var result = turnos.GroupBy(m => m.Fecha?.ToString("yyyy-MM-dd"))
            ...
            if (listarBalanzaManual != null && listarBalanzaManual.Count > 0)
            {
                foreach (var balanza in listarBalanzaManual)
                {
                    ...
                    {
                        var turno = turnoPorFecha.Turnos.Where(x => x.Turno.Id == balanza.TurnoPuerto.Id).FirstOrDefault();
                        if (turno == null)
                        {
        ...
        {
            var result = new List<BalanzasCortesDto>();
            try
            {
                var lineUp = repositorio.Obtener<LineUp>(x => x.ModuloDeCarga.Id == IdModuloDeCarga);
                ...
                if (lineUp != null)
                {
                    var embarque = repositorio.Obtener<Embarque>(x => x.Id == lineUp.Embarque.Id);
                    result = Listar<BalanzasCortes, BalanzasCortesDto>(x => x.ModuloDeCarga_id == IdModuloDeCarga).Where(x => x.Fecha_Inicio >= embarque.FechaHoraInicioCarga).OrderByDescending(x => x.Fecha_Inicio).ToList();
                }
        ...
        public void GuardarBalanzaCorte(List<BalanzasCortesDto> balanzasCortesDtos)
        {
            try
            {
                BalanzasCortes balanzasCortes_db;
                foreach (var item in balanzasCortesDtos)
                {
                    balanzasCortes_db = repositorio.Obtener<BalanzasCortes>(x => x.Id == item.Id);
                    ...
                    ModuloDeCargaPlanillaDeTurnosCortes moduloDeCargaPlanillaDeTurnosCortes = repositorio.Obtener<ModuloDeCargaPlanillaDeTurnosCortes>(x => x.idBalanzaCorte == balanzasCortes_db.Id);
                    if (moduloDeCargaPlanillaDeTurnosCortes == null)
                    {
                        int id_turno = (balanzasCortes_db.Fecha_Inicio.Value.Hour / 6) + 1;
                        ModuloDeCargaPlanillaDeTurnos moduloDeCargaPlanillaDeTurnos = CrearModuloDeCargaPlanillaDeTurnos(balanzasCortes_db.ModuloDeCarga_id, Convert.ToDateTime(balanzasCortes_db.Fecha_Inicio), id_turno);
                        moduloDeCargaPlanillaDeTurnosCortes = new ModuloDeCargaPlanillaDeTurnosCortes();
                        moduloDeCargaPlanillaDeTurnosCortes.idBalanzaCorte = balanzasCortes_db.Id;
                        moduloDeCargaPlanillaDeTurnosCortes.ModuloDeCargaPlanillaDeTurnos = moduloDeCargaPlanillaDeTurnos;
                        moduloDeCargaPlanillaDeTurnosCortes.Observaciones = balanzasCortes_db.Observaciones;
                        moduloDeCargaPlanillaDeTurnosCortes.HoraFin = balanzasCortes_db.Fecha_Corte.Value.ToString("HH:mm");
                        moduloDeCargaPlanillaDeTurnosCortes.HoraInicio = balanzasCortes_db.Fecha_Inicio.Value.ToString("HH:mm");
           ...
           Exportador exportador, int cantidad, int idBalanzaCorte = 0)
        {
            try
            {
                ModuloDeCargaPlanillaDeTurnosDetallesSolido moduloSolido = repositorio.Listar<ModuloDeCargaPlanillaDeTurnosDetallesSolido>(x => x.ModuloDeCargaPlanillaDeTurnos.Id == planilla.Id &&
                x.Bodega.Id == bodega.Id && x.Exportador.Id == exportador.Id && x.Destino.Id == destino.Id && x.idBalanzaCorte == idBalanzaCorte).FirstOrDefault();

                if (moduloSolido != null)
                {
                    moduloSolido.Cantidad += cantidad;
                }
                else
                {
                    moduloSolido = new ModuloDeCargaPlanillaDeTurnosDetallesSolido();
        ...
        public void CrearModuloDeCargaPlanillaDeTurnosCortes(ModuloDeCargaPlanillaDeTurnos turno, BalanzasCortes bc)
        {
            try
            {
                ModuloDeCargaPlanillaDeTurnosCortes ModuloDeCargaPlanillaDeTurnosCortes = new ModuloDeCargaPlanillaDeTurnosCortes()
                {
                    HoraFin = bc.Fecha_Corte.Value.ToString("HH:mm"),
                    HoraInicio = bc.Fecha_Inicio.Value.ToString("HH:mm"),
                    idBalanzaCorte = bc.Id,
                    ModuloDeCargaPlanillaDeTurnos = turno,
                    MotivosDeCorte = repositorio.Obtener<MotivosFallasBalanza>(x => x.Id == bc.MotivosFallasBalanza_id),
                    Observaciones = bc.Observaciones,
                    TiempoTotal = Convert.ToDateTime(bc.Fecha_Corte.Value.Subtract(bc.Fecha_Inicio.Value).ToString()).ToString("HH:mm")
                };
        ...
        public Dictionary<string, string> ObtenerInformacionCortesBalanzas(int IdModuloDeCarga)
        {
            try
            {
                ...
                if (lineup != null)
                {
                    ...
                    int embarqueId = embarque.Id;

                    var ObjetoEmbarque = repositorio.Obtener<Embarque>(x => x.Id == embarqueId);
        ...
        public Dictionary<string, string> ObtenerRitmosBalanzas78(int IdModuloDeCarga, int numeroBalanza)
        {
            try
            {
                ...
                int cargoHastaAhora = 0, ritmoDeEmbarque = 0, idUltimaBalanzadaEnCurso = 0;
                string numeroBalanzaStr = numeroBalanza.ToString();
                double tiempoDeCarga = 0;
                bool tieneBalanzadaEnCurso = false;
                ...
                if (lineup != null)
                {
                    ...
                    // Obtenemos la carga en curso
                    var cargasEnCurso = repositorio.Listar<Carga>(x => x.Vapor.Id == idVapor &&
                                                                       x.FechaInicio == null &&
                                                                       x.CargaOpuesta_Id == null &&
                                                                       x.NumeroBalanza == numeroBalanzaStr);

                    if (cargasEnCurso.Count() > 0)
                    {
                        int? idCargaEnCurso = cargasEnCurso[0].Id;
                        idUltimaBalanzadaEnCurso = repositorio.Listar<Balanzada>(x => x.CargaInicial_Id == idCargaEnCurso &&
                                                                                      x.NumeroBalanza == numeroBalanzaStr).LastOrDefault().Id;

                        if (idUltimaBalanzadaEnCurso != null) tieneBalanzadaEnCurso = true;
                    }
                    ...
                    {
                        totalPesoNeto += repositorio.Listar<Balanzada>(x => x.CargaInicial_Id == cargaEnCurso.Id && x.NumeroBalanza == numeroBalanzaStr).Sum(x => x.PesoNeto);
                    }
                    ...
                    if (tieneBalanzadaEnCurso)
                    {
                        // Busco fecha de fin
                        DateTime fechaUltimaBalanzadaEnCurso = repositorio.Obtener<RegistroBalanzaPuerto>(x => x.Id == idUltimaBalanzadaEnCurso &&
                                                                                                          x.NumeroBalanza == numeroBalanzaStr).Fecha;

                        ultimaBalanzada = fechaUltimaBalanzadaEnCurso;
                        tiempoDeCarga = ((fechaUltimaBalanzadaEnCurso - fechaInicialBalanza).TotalMinutes) / 60;
                        arranco = fechaInicialBalanza;
                    }
                    else
                    {
                        List<Carga> UltimaCarga = repositorio.Listar<Carga>(x => x.Vapor.Id == idVapor &&
                                                                                 x.FechaInicio >= embarqueBase.FechaHoraInicioCarga &&
                                                                                 x.NumeroBalanza == numeroBalanzaStr).ToList();

                        int? idUltimaCarga = UltimaCarga != null && UltimaCarga.Count > 0 ? UltimaCarga.Last().CargaOpuesta_Id : 0;

                        if (idUltimaCarga != null)
                        {
                            Balanzada UltimaBalanzada = repositorio.Listar<Balanzada>(x => x.CargaInicial_Id == idUltimaCarga &&
                                                                                           x.NumeroBalanza == numeroBalanzaStr).LastOrDefault();

                            if (UltimaBalanzada != null)
                            {
                                int idUltimaBalanzada = UltimaBalanzada != null && UltimaBalanzada.Id > 0 ? UltimaBalanzada.Id : 0;
        ...
        public Dictionary<string, int> ObtenerRitmos(int modulodecarga_id)
        {
            try
            {
                ...
                var embarque = repositorio.ObtenerProyeccion<LineUp, Embarque>(x => x.ModuloDeCarga.Id == modulodecarga_id, x => x.Embarque);
                if (embarque.FechaHoraInicioCarga == null)
                {
                ...
                var finesDeCarga = repositorio.Listar<Carga>(x => x.FechaInicio != null && x.Vapor.Id == vapor_id && x.FechaInicio > fechaInicioFinal);
                foreach (var fin in finesDeCarga)
                {
                    totalCargado += fin.ToneladasAW;

                    var fechainicio = repositorio.ObtenerProyeccion<RegistroBalanzaPuerto, DateTime?>(x => x.Id == fin.CargaOpuesta_Id && x.NumeroBalanza == fin.NumeroBalanza, x => x.Fecha);
                    var fechaFin = repositorio.ObtenerProyeccion<RegistroBalanzaPuerto, DateTime?>(x => x.Id == fin.Id && x.NumeroBalanza == fin.NumeroBalanza, x => x.Fecha);
                    tiempoCarga += (fechaFin - fechainicio).GetValueOrDefault().TotalMinutes;
                }

                var cargasEnCurso = repositorio.Listar<Carga>(x => x.Vapor.Id == vapor_id && x.FechaInicio == null && x.CargaOpuesta_Id == null);
                foreach (var cargaEnCurso in cargasEnCurso)
                {
                    totalCargado += repositorio.Sumar<Balanzada>(x => x.PesoNeto, x => x.CargaInicial_Id == cargaEnCurso.Id && x.NumeroBalanza == cargaEnCurso.NumeroBalanza); // Sumo los pesos de balanzadas ya que aun no hay un registro de fin
                    var fechaInicio = repositorio.ObtenerProyeccion<RegistroBalanzaPuerto, DateTime>(x => x.Id == cargaEnCurso.Id && x.NumeroBalanza == cargaEnCurso.NumeroBalanza, x => x.Fecha);
                    tiempoCarga += (DateTime.Now - fechaInicio).TotalMinutes; // Sumo la diferencia entre el inicio de la carga abierta y el momento actual
                }

                totalCargado = totalCargado / 1000;
                ritmosDeCarga["totalCargado"] = (int)totalCargado;
                ...
                // como ritmo carga NETO no contempla bajas cargas ni fuleos, me traigo los datos del corte cuando sea == a BCB(Baja carga Buque) o F(Fuleos) y retorno el valor restandolo a el Ritmo neto
                ...
                var tnNeto = totalCargado - bajaCargas.Sum(bc => bc.Tn ?? 0);
                var tiempoCargaNeto = tiempoCarga - bajaCargas.Sum(bc => (bc.Fecha_Corte - bc.Fecha_Inicio).GetValueOrDefault().TotalMinutes);


                double ritmoDeCarga = ((double)totalCargado * 60) / tiempoCarga;
                double ritmoCargaNeto = ((double)tnNeto * 60) / tiempoCargaNeto;
                ritmosDeCarga["ritmoDeCarga"] = (int)ritmoDeCarga;
                ritmosDeCarga["ritmoCargaNeto"] = (int)ritmoCargaNeto;
        ...
        public Dictionary<string, double> ObtenerRitmosLiquidos(int modulodecarga_id)
        {
            var planillaDeTurnos = repositorio.Listar<ModuloDeCargaPlanillaDeTurnos>(x => x.ModuloDeCarga.Id == modulodecarga_id);
            var cargas = planillaDeTurnos.SelectMany(pt => pt.ModuloDeCargaPlanillaDeTurnosDetallesLiquido);
            var cortes = planillaDeTurnos.SelectMany(pt => pt.ModuloDeCargaPlanillaDeTurnosCortes).Where(d => d.MotivosDeCorte.Siglas != "BCP" && d.MotivosDeCorte.Siglas != "BCB");
            var bajasCargas = planillaDeTurnos.SelectMany(pt => pt.ModuloDeCargaPlanillaDeTurnosCortes).Where(d => d.MotivosDeCorte.Siglas == "BCP" || d.MotivosDeCorte.Siglas == "BCB");

            var cantTotal = (double)cargas.Sum(c => c.Cantidad);
            var cantidadBC = (double)bajasCargas.Sum(c => c.Cantidad);
            ...
            var horasBruto = tiempoTotal.TotalMinutes / 60d;
            var horasBC = tiempoBC.TotalMinutes / 60d;
            var horasCortes = tiempoCortes.TotalMinutes / 60d;
            var horasNeto = horasBruto - horasBC - horasCortes;

            var ritmoBruto = horasBruto > 0 ? cantTotal / horasBruto : 0;
            var ritmoNeto = horasNeto > 0 ? cantidadNeto / horasNeto : 0;

            Dictionary<string, double> ritmosDeCargaLiquidos = new Dictionary<string, double>
            {
                { "LlevasCargado", cantTotal },
                { "RitmoAcumulado", Math.Round(ritmoBruto, 2) },
                { "RitmoAcumuladoNeto", Math.Round(ritmoNeto, 2) }
            };
		...
		public List<HistorialDeBuquesDto> ListarHistorialDeBuques(int anio, int mes, int vaporId, string nombreBuque, string destino, string exportador, string controlPrivado, DateTime? desde = null, DateTime? hasta = null, List<string> producto = null)
		{
			...
			if (listaEmbarques != null)
			{
				if (listaEmbarques.Count > 0)
				{
					foreach (var embarque in listaEmbarques)
					{
						...
						if (listaBuquesLineuUp != null && listaBuquesLineuUp.Count > 0)
						{
							foreach (var lineUp in listaBuquesLineuUp)
							{
								...
								foreach (var planillaDeTurnos in moduloCarga.ModuloDeCargaPlanillaDeTurnos)
								{
									...
									if (lineUp != null && lineUp.PlanoDeCarga != null && lineUp.PlanoDeCarga.AgentesControlPrivado != null && lineUp.PlanoDeCarga.AgentesControlPrivado.Count > 0)
									{
									...
									log.Debug("Modulo de carga de periodo de carga");
									if (moduloCarga.ModuloDeCargaPeriodoDeCarga != null && moduloCarga.ModuloDeCargaPeriodoDeCarga.Count > 0)
									{
								...
								{
									Dictionary<string, double> resultado = ObtenerRitmosLiquidos(lineUp.ModuloDeCarga.Id);
									if (resultado != null)
									{
								...
								if (lineUp != null && !lineUp.Embarque.EsLiquido)
								{
									...
									{
										ritmoCargaNeto = ObtenerRitmoCargaNeto(lineUp.ModuloDeCarga.Id);
									}
									else
									{
        ...
        public int obtenerPlanoDeCargaId(int idEmbarque)
        {
            try
            {
        ...
        private string DevolverFechaHoraConcatenada(DateTime? fecha, string hora)
        {
            ...
            return fecha.Value.ToString("dd/MM/yyyy") + "-" + hora.Trim();
        }
        public Dictionary<string, string> ObtenerRegistroFechas(int idEmbarque)
        {
            #region Variables
            string etaRecalada = "-";
            string obligacionCarga = "-";
            string amarro = "-";
            string habilitacion = "-";
            string limpiezaDesde = "-";
            ...
            try
            {
                Dictionary<string, string> registrosFechas = new Dictionary<string, string>();
                //Obtengo el lineUp con el embarque ID, ya que el mismo tiene asociados el Embarque y el Modulo de carga
                LineUp lineUp = repositorio.Obtener<LineUp>(x => x.Embarque.Id == idEmbarque);
                #region EtaRecalada
                ...
                {
                    var arrObligacionCarga = lineUp.Embarque.ObligacionCarga.Value.ToString("dd/MM/yyyy");
                }
                #endregion
                ...
                else
                {
                    ...
                    comienzoCarga = turno.Fecha.Value.ToString("dd/MM/yyyy-HH:mm");
                }
                #endregion
                ...
                if (periodoDeCarga.FechaDesamarro != null && periodoDeCarga.HoraDesamarro == null)
                {
                    ...
                    desamarre = arrFechaDesamarro[0];
                }
                else if (periodoDeCarga.FechaDesamarro != null && periodoDeCarga.HoraDesamarro != null)
                {
                    var arrFechaDesamarro = periodoDeCarga.FechaDesamarro.ToString().Split(' ');
                ...
                if (amarro != "-" && !string.IsNullOrEmpty(amarro) && desamarre != "-" && !string.IsNullOrEmpty(desamarre))
                {
                    DateTime dateAmarro = DateTime.ParseExact(amarro, "dd/MM/yyyy-HH:mm", CultureInfo.InvariantCulture);
                    DateTime dateDesamarre = DateTime.ParseExact(desamarre, "dd/MM/yyyy-HH:mm", CultureInfo.InvariantCulture);
                    hsEnPuerto = (dateDesamarre - dateAmarro).TotalHours.ToString();
                }
                #endregion
                #region SetDictionary
                //seteo el dictionary con los datos obtenidos.
                registrosFechas.Add("etaRecalada", etaRecalada);
                registrosFechas.Add("ObligacionCarga", obligacionCarga);
                registrosFechas.Add("Amarre", amarro);
                registrosFechas.Add("Habilitacion", habilitacion);
                registrosFechas.Add("LimpiezaDesde", limpiezaDesde);
        ...
        public Dictionary<string, object> ObtenerIdsUsuales(int idEmbarque)
        {
            ...
            try
            {
                LineUp lineUp = repositorio.Obtener<LineUp>(x => x.Embarque.Id == idEmbarque);
                //moduloDeCarga ID
                if (lineUp.ModuloDeCarga != null)
                {
                    idModuloDeCarga = lineUp.ModuloDeCarga.Id;
                }
                if (lineUp.PlanoDeCarga != null)
                {
                    idPlanoDeCarga = lineUp.PlanoDeCarga.Id;
                }
                if (lineUp.Embarque.Vapor != null)
                {
        ...
        public bool EliminarArchivos(List<ArchivosPuertoDto> archivosPuerto)
        {

            foreach (var archivo in archivosPuerto)
            {
                var archivoDb = repositorio.Obtener<ArchivosPuerto>(x => x.Id == archivo.Id);
                repositorio.Remover(archivoDb);
            }
        ...
        public int GuardarTipoArchivo(TipoArchivoPuertoDto tipoArchivoPuertoDto)
        {
            TipoArchivoPuerto tipoarchivoDB = repositorio.Obtener<TipoArchivoPuerto>(x => x.TipoArchivo == tipoArchivoPuertoDto.TipoArchivo);

            if (tipoarchivoDB == null)
            {
                tipoarchivoDB = new TipoArchivoPuerto()
                {
                    TipoArchivo = tipoArchivoPuertoDto.TipoArchivo
                };

                repositorio.Agregar(tipoarchivoDB);
                repositorio.GuardarCambios();

            }

            return tipoarchivoDB.Id;
        }
        ...
        public EmbarqueInformacionDto obtenerEmbarqueInformacion(int idEmbarque)
        {
            try
            {
                var infoEmbarque = Obtener<EmbarqueInformacion, EmbarqueInformacionDto>(x => x.Embarque.Id == idEmbarque);
                return infoEmbarque;
            }
            catch (Exception ex)
            {
        ...
        private void CompletarDatosHistorialDeEmbarque(List<HistorialDeBuquesDto> historialDeBusquesDto)
        {
            if (historialDeBusquesDto != null && historialDeBusquesDto.Count > 0)
            {
                foreach (var item in historialDeBusquesDto)
                {
                    ...
                    if (item.ModuloDeCargaId > 0 && item.NombreMuelle == "San Benito")
                    {
                        ...
                        {
                            Dictionary<string, double> resultado = ObtenerRitmosLiquidos(item.ModuloDeCargaId);
                            if (resultado != null)
                            {
                                if (resultado.Count > 0)
                                {
                        ...
                        if (item != null && !item.EsLiquido)
                        {
                            ...
                            {
                                item.TotalRitmoNormal = ObtenerRitmoCargaNeto(item.ModuloDeCargaId);
                            }
                            else
                            {
        ...
        public IList<HistorialDeBuquesDto> ListarHistorialDeEmbarques(int vaporId, string nombreBuque, string destino, string exportador, string controlPrivado, DateTime? desde = null, DateTime? hasta = null, List<string> producto = null, List<string> muelle = null, Paginacion paginacion = null)
        {
            try
            {
                var fechaHasta = hasta.HasValue ? new DateTime(hasta.Value.Year, hasta.Value.Month, DateTime.DaysInMonth(hasta.Value.Year, hasta.Value.Month)) : (DateTime?)null;
                ...
                var historial = repositorio.ListarConsultaPaginada(new ListarHistorialDeEmbarquesConsulta(vaporId, nombreBuque, destino, exportador, controlPrivado, desde, fechaHasta, producto, muelle, paginacion)).ToList();
        ...
        public BalanzaManualDto ObtenerBalanzaManual(int id)
        {
            ...
            var turnos = Listar<TurnoPuerto, TurnoPuertoDto>();
            TurnoPuertoDto turnoPuertoDto;
            var horas = balanzaManualDto.HoraInicio.Split(':');
            int horaInicio = Convert.ToInt32(horas[0]);
            turnoPuertoDto = turnos.Where(item => horaInicio >= Convert.ToInt32(item.Nombre.Substring(0, 2)) && horaInicio < Convert.ToInt32(item.Nombre.Substring(3, 2))).FirstOrDefault();
            balanzaManualDto.TurnoPuerto = turnoPuertoDto;
        ...
        public BalanzaManualDto GuardarBalanzaManual(BalanzasCortesDto dto, string nombreUsuario)
        {
            ...
            {
                log.Error("Error en metodo GuardarBalanzaManual: {0}\n{1}", e.Message, e.StackTrace);
        ...
        {
            var moduloDeCarga = repositorio.Obtener<ModuloDeCarga>(moduloCargaId) ?? throw new Exception("No se ha encontrado el modulo de carga con Id " + moduloCargaId);
            IEnumerable<ModuloDeCargaPlanillaDeTurnosDetallesSolido> detalles;
            IEnumerable<int> cortesBajasCargasIds;
            DateTime inicio, fin;
            decimal ritmoBalanza7, ritmoBalanza8, ritmoBruto, ritmoNeto;

            var periodoCarga = moduloDeCarga.ModuloDeCargaPeriodoDeCarga.First();
            var horaInicio = TimeSpan.Parse(periodoCarga.HoraComienzoCarga);
            inicio = periodoCarga.FechaComienzoCarga.Value.Add(horaInicio);

            if (esCalculoGeneral)
            {
                detalles = moduloDeCarga.ModuloDeCargaPlanillaDeTurnos.SelectMany(t => t.ModuloDeCargaPlanillaDeTurnosDetallesSolido);
                cortesBajasCargasIds = moduloDeCarga.ModuloDeCargaPlanillaDeTurnos.SelectMany(t => t.ModuloDeCargaPlanillaDeTurnosCortes).Select(c => c.idBalanzaCorte.Value);

                if (periodoCarga.FechaFinalizacionCarga.HasValue)
                {
                    var horaFin = TimeSpan.Parse(periodoCarga.HoraFinalizacionCarga);
                    fin = periodoCarga.FechaFinalizacionCarga.Value.Add(horaFin);
                }
                else
                {
                    // En caso de no tener fecha de fin de carga, se toma como fin el final del ultimo turno que tenga cargas
                    var ultimoTurno = moduloDeCarga.ModuloDeCargaPlanillaDeTurnos
                        .Where(t => t.ModuloDeCargaPlanillaDeTurnosDetallesSolido.Count > 0)
                        .OrderByDescending(t => t.Fecha.Value.Date)
            ...
            {
                var turno = moduloDeCarga.ModuloDeCargaPlanillaDeTurnos.FirstOrDefault(t => t.Fecha.Value.Date == fechaTurno.Value.Date && t.TurnoPuerto.Orden == turnoId);

                detalles = turno.ModuloDeCargaPlanillaDeTurnosDetallesSolido;
                cortesBajasCargasIds = turno.ModuloDeCargaPlanillaDeTurnosCortes.Select(c => c.idBalanzaCorte.Value);

                // Intervalo de carga
                // Calculo de hora de inicio y fin de turno segun el orden:
                // fin      = orden * 6hs.  Ej: Turno 3 =>  3 * 6   = 18hs
                ...
                // Si no es el primer turno, se toma la hora inicial del turno
                if (!esPrimerTurno)
                {
                    inicio = turno.Fecha.Value.Date.AddHours(turno.TurnoPuerto.Orden * 6 - 6);
                }

                if (esUltimoTurno && periodoCarga.FechaFinalizacionCarga.HasValue)
                {
                    var horaFin = TimeSpan.Parse(periodoCarga.HoraFinalizacionCarga);
                    fin = periodoCarga.FechaFinalizacionCarga.Value.Add(horaFin);
                }
                else // Si no es el ultimo, o si no hay fecha de fin de carga, se toma la hora final del turno
                {
                    fin = turno.Fecha.Value.Date.AddHours(turno.TurnoPuerto.Orden * 6);
                }
            }
            ...
            if (totalMinutosNeto > 0)
            {
                ritmoNeto = (decimal)((kgCargaNormal / 1000D) / (totalMinutosNeto / 60D));
            }

            return new RitmoDeCargasBalanzasDto
            {
                ArrancoBalanza7 = inicio,
                ArrancoBalanza8 = inicio,
                UltimaBalanzada7 = fin,
                UltimaBalanzada8 = fin,
                UltimaActualizacionBalanza7 = DateTime.Now,
                UltimaActualizacionBalanza8 = DateTime.Now,
                CargaBalanza7 = Math.Round(kgTotalBalanza7 / 1000M, 2),
                CargaBalanza8 = Math.Round(kgTotalBalanza8 / 1000M, 2),
                RitmoBalanza7 = Math.Round(ritmoBalanza7, 2),
                RitmoBalanza8 = Math.Round(ritmoBalanza8, 2),
                LLevasCargando = Math.Round(kgTotal / 1000M, 2),
                RitmoCargaBruto = Math.Round(ritmoBruto, 2),
                RitmoCargaNeto = Math.Round(ritmoNeto, 2)
            };
        }

        private decimal ObtenerRitmoCargaNeto(int moduloDeCargaId)
        {
            var ritmos = ObtenerRitmosCargaManual(moduloDeCargaId, true, null, null);
            return ritmos.RitmoCargaNeto;
        }
        ...
        private void CrearBalanzaManualCortes(BalanzasCortesDto dto, string nombreUsuario)
        {
            var listaRegistroCortes = this.ObtenerBalanzasFechasCortesPorTurno(dto);
            var balanzaCortes = new BalanzasCortes();
            foreach (var registroCortes in listaRegistroCortes)
            {
                balanzaCortes = new BalanzasCortes()
                {
                    Cerrado = dto.Cerrado,
                    Fecha_Inicio = Convert.ToDateTime(string.Format("{0} {1}", registroCortes.FechaInicio, registroCortes.HoraInicio)),
                    Fecha_Corte = Convert.ToDateTime(string.Format("{0} {1}", registroCortes.FechaCorte, registroCortes.HoraCorte)),
                    Kg = dto.Kg,
                    NumeroBalanza = dto.NumeroBalanza,
                    Observaciones = dto.Observaciones,
                    Material_id = dto.Material_id,
        ...
        private List<BalanzasFechasCortesRegistroDto> ObtenerFechasCorteRegistro(DateTime fechaInicio, DateTime fechaFin)
        {
            List<BalanzasFechasCortesRegistroDto> listaFechasCortePuerto = new List<BalanzasFechasCortesRegistroDto>();
            var diferenciaDia = fechaFin.Date - fechaInicio.Date;
            var diferenciaDias = diferenciaDia.TotalDays == 0 ? 1 : diferenciaDia.TotalDays + 1;
            if (diferenciaDias == 1)
            {
                listaFechasCortePuerto.Add(new BalanzasFechasCortesRegistroDto()
                {
            ...
            {
                for (int inicio = 0; inicio < diferenciaDias; inicio++)
                {
                    if (inicio == 0)
                    {
                        listaFechasCortePuerto.Add(new BalanzasFechasCortesRegistroDto()
                        {
                            FechaInicio = fechaInicio,
                            FechaFin = fechaInicio.Date.AddDays(1).AddMinutes(-1)
                        });
                    }
                    else
                    {
                        var fechaAdicionada = fechaInicio.Date.AddDays(inicio);
                        if (fechaAdicionada.Date == fechaFin.Date)
                        {
                            listaFechasCortePuerto.Add(new BalanzasFechasCortesRegistroDto()
                            {
                        ...
                        else
                        {
                            listaFechasCortePuerto.Add(new BalanzasFechasCortesRegistroDto()
                            {
                                FechaInicio = fechaAdicionada.Date,
                                FechaFin = fechaAdicionada.Date.AddDays(1).AddMinutes(-1)
                            });
                        }
                    }
                }
            ...
            return listaFechasCortePuerto;
        }
        private List<BalanzasFechasCortesPorTurnoDto> ObtenerBalanzasFechasCortesPorTurno(BalanzasCortesDto dto)
        {
            ...
            foreach (var fechaCortePuerto in listaFechasCortePuerto)
            {
                ...
                var horaFinSel = fechaCortePuerto.FechaFin.ToString("HH:mm").Split(':');
                ...
                {
                    int inicioTurno = turnoPuertoInicio.Orden;
                    int finTurno = turnoPuertoFin.Orden;

                    while (finTurno >= inicioTurno)
                    {
                        var turnoSeleccionado = listaTurnosPuerto.Where(x => x.Orden == inicioTurno).FirstOrDefault();
                        ...
                        else if (turnoSeleccionado.Orden == turnoPuertoFin.Orden && fechaCortePuerto.FechaInicio.Date == dto.Fecha_Corte.Value.Date && horaInicioSeleccionado != horaFinSeleccionado)
                        {
                            ...
                            {
                                TurnoPuerto = Obtener<TurnoPuerto, TurnoPuertoDto>(x => x.Orden == turnoSeleccionado.Orden),
                                FechaInicio = fechaCortePuerto.FechaFin.ToString("yyyy-MM-dd"),
                                HoraInicio = horaInicioSeleccionado,
                                FechaCorte = fechaCortePuerto.FechaFin.ToString("yyyy-MM-dd"),
                                HoraCorte = horaFinSeleccionado,
                            });
        ...
        private void GuardarPlanillaDeTurnoCortes(BalanzasCortes balanzaCortes, BalanzasCortesDto dto, BalanzaManualDto balanzaManual)
        {
            ...
            if (turno == null)
            {
                turno = new ModuloDeCargaPlanillaDeTurnos
                {
                    ...
                    EsLiquido = false,
                    Fecha = registroBalanzaCorte.Fecha_Inicio,
                    Enviado = false,
                    Cerrado = false,
        ...
        public Dictionary<string, decimal> ObtenerRitmosBalanzaManual(int modulodecarga_id)
        {
            ...
            var ritmos = ObtenerRitmosCargaManual(modulodecarga_id, true, null, null);
            var planoDeCarga = repositorio.Obtener<LineUp>(l => l.ModuloDeCarga.Id == modulodecarga_id).PlanoDeCarga;
        ...
        public void ActualizarHorariosExportadorSolidos(int moduloDeCargaId)
        {
            try
            {
                ...
                var cargas = turnos
                     ...
                     .GroupBy(detalle => new { detalle.Exportador, detalle.MaterialPuerto, detalle.Destino })
                ...
                {
                    var horarioBd = horariosBd.FirstOrDefault(h => h.Exportador?.Id == carga.Exportador.Id &&
                    h.MaterialPuerto?.Id == carga.MaterialPuerto.Id && h.Destino?.Id == carga.Destino.Id);
                    if (horarioBd == null)
                    {
                        var nuevoHorario = new HorariosExportador
                        {
        ...
        public void ActualizarHorariosExportadorLiquidos(int moduloDeCargaId)
        {
            try
            {
                ...
                var cargas = turnos
                     ...
                     .GroupBy(detalle => new { detalle.Exportador, detalle.MaterialPuerto, detalle.Destino, detalle.BodegaParcel })
                     .Select(grupo => grupo.First())
                     .ToList();
                ...
                {
                    var horarioBd = horariosBd.FirstOrDefault(h => h.Exportador?.Id == carga.Exportador.Id &&
                    h.MaterialPuerto?.Id == carga.MaterialPuerto.Id && h.Destino?.Id == carga.Destino.Id && h.BodegaParcel == carga.BodegaParcel);
                    if (horarioBd == null)
                    {
                        var nuevoHorario = new HorariosExportador
                        {
        ...
        private void AsignarCantidadAHorarios(IList<HorariosExportadorDto> horarios, IList<ModuloDeCargaPlanillaDeTurnos> turnos, bool esLiq)
        {
            ...
            {
                var detalles = turnos
               .SelectMany(turno => turno.ModuloDeCargaPlanillaDeTurnosDetallesSolido).ToList();
        ...
        public void ReabrirTurnoModuloDeCarga(int idTurno, string username)
        {
            ...
            {
                Pantalla = "ReabrirTurnoModuloDeCarga",
                Usuario = username,
                Fecha = DateTime.Now,
        ...
        public void CerrarTurnoLiquido(int idTurno, string username)
        {
            var turno = this.repositorio.Obtener<ModuloDeCargaPlanillaDeTurnos>(idTurno);
            turno.Cerrado = true;
            turno.GuardadoPorRecibidor = true;
            var turnoDto = conversor.Convertir<ModuloDeCargaPlanillaDeTurnos, ModuloDeCargaPlanillaDeTurnosDto>(turno);
            var logEdicion = new LogABM
            {
                Pantalla = "CerrarTurnoLiquido",
                Usuario = username,
                Fecha = DateTime.Now,
                Evento = EventoABM.Modificacion,
                Entidad = turnoDto.ToJson(),
