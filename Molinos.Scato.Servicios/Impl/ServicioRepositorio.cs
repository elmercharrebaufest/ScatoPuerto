...
namespace Molinos.Scato.Servicios.Impl
{
    public class ServicioRepositorio : IServicioRepositorio
    {
        ...
        public ServicioRepositorio(IRepositorio repositorio, IConversor conversor, ILogger log, IFirmaProvider firmaProvider,
            ICalculadoraDescuento calculadora, IConfiguracionProvider configuracion, IServicioOrquestador servicioOrquestador, ZSDWS_SCATO servicioSap)
        {
        ...
        public CamaraDto ObtenerCamaraPorMaterialPorCentro(Guid instanceId)
        {
            var materialCentro = repositorio.ObtenerProyeccion((Recorrido x) => x.InstanciaWorkflow == instanceId,
                                          x => new { MaterialId = x.Material.Id, CentroId = x.Centro.Id });
            ...
            return camara != null ? conversor.Convertir<Camara, CamaraDto>(camara) : null;
        }
        ...
        public IList<MaterialPorCentroDto> BuscarMaterialesPorCentro(int centroId, string filtro)
        {
            return
                Listar<MaterialPorCentro, MaterialPorCentroDto>(
                    m => m.Material.Activo && m.Centro.Id == centroId && m.Material.Descripcion.Contains(filtro), 20);
        }
        ...
        public IList<CamaraDto> ListarCamaras()
        {
            return Listar<Camara, CamaraDto>();
        }

        public ListaPaginada<CamaraDto> ListarPaginadoCamaras(string filtro, Paginacion paginacion)
        {
            ...
            return Listar<Camara, CamaraDto>(expresionFiltro, paginacion);
        }
        ...
        public CamaraDto ObtenerCamara(int id)
        {
            return Obtener<Camara, CamaraDto>(id);
        }
        ...
        public CartaPorteValidaResponseDto NumeroCartaPorteValido(string numero, int centroId, string workflowCodigo)
        {
        ...
        {
            return
                Listar<Entregador, EntregadorDto>(f => f.RazonSocial.Contains(criteria) || f.Cuil.Contains(criteria), 20);
        }
        ...
        {
            var calle = repositorio.Obtener<Calle>(id);
            return calle != null ? calle.Nombre : "";
        }
        ...
        private ImpresionReciboMunicipalRecorridoDto ObtenerImpresionReciboMunicipal(Expression<Func<Recorrido, bool>> condicion)
        {
            ...
            var reciboMunicipal = repositorio.ObtenerMayor<ReciboMunicipal, int>(x => x.Centro.Id == recorrido.Centro.Id && hoy >= x.FechaActivacion, x => x.Id);
        ...
        public CamaraDto ObtenerCamaraDeExcepcionDescuento(Guid instanceId, int materialId, int centroId)
        {
            ...
            return camara == null ? null : conversor.Convertir<Camara, CamaraDto>(camara);
        }
        ...
        public GraficoEficienciaHidraulicasDto ObtenerGraficoDeEficienciaHidraulicas(GraficoEficienciaHidraulicasDto model, int centroId)
        {
            model = repositorio.ObtenerConsultaEscalar(new EficienciaHidraulicasConsulta(model, centroId));
            return model;
        }
        ...
        {
            var nroCp = repositorio.ObtenerProyeccion((Recorrido x) => x.InstanciaWorkflow == instanceId, x => new { x.NumeroDocumentoIngreso, Cupo = x.Vehiculo != null ? x.Vehiculo.CartaPorte.Cupo : null });
            var carga = repositorio.ObtenerMayor<CargaDeCupo, int>(x => x.NumeroCartaPorte == nroCp.NumeroDocumentoIngreso || (x.Cupo != "MOL1111/11111111" && x.Cupo == nroCp.Cupo), x => x.Id);
        ...
        {
            return !repositorio.Existe<MaterialPorCentro>(x => x.Centro.Id == idCentro && !x.ImprimeReciboMunicipal);
        }
