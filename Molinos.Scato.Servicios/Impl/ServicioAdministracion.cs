...
namespace Molinos.Scato.Servicios.Impl
{
    public class ServicioAdministracion : IServicioAdministracion
    {
        ...
        /// <summary>
        /// Obtiene el detalle de provisiones para un producto y periodo determinado. Soporta filtros por muelle, embarque, exportador y acuerdo.
        /// </summary>
        public ExcelProvisionGastoDatosDto ObtenerDatosProvisionesExcel(int productoId, DateTime periodo, int? muelleId, int? embarqueId, int? exportadorId, int? acuerdoId)
        {
            const int idMOA = 77;
            var lineups = ObtenerLineupsExcel(periodo, muelleId, embarqueId);

            var tarifasProducto = _repositorio.Incluir<TarifaPorProducto>()
                .Where(t => t.MaterialPuerto.Id == productoId && t.Periodo.Year == periodo.Year && t.Periodo.Month == periodo.Month)
                .FirstOrDefault();


            var datosExcel = new List<ProvisionGastoDatosExcelDto>();

            foreach (var lineup in lineups)
            {
                var esSanBenito = lineup.Embarque.SanBenito || lineup.Embarque.Muelle?.Id == 1;

                var cantidadesExportadores = ExtraerCargas(lineup, productoId, exportadorId);
            ...
            var tarifaDolar = _repositorio.Obtener<TarifaCotizacionDolar>(t => t.Periodo.Year == periodo.Year && t.Periodo.Month == periodo.Month);
            var producto = _repositorio.Incluir<MaterialPuerto>().Where(mp => mp.Id == productoId).FirstOrDefault()?.Descripcion;

            return new ExcelProvisionGastoDatosDto
            {
                DatosExcel = datosExcel,
                CotizacionDolar = tarifaDolar?.ValorDolar ?? 0,
                Producto = producto,
                Periodo = periodo.ToString("MMMM-yy")
            };
        }
        ...
        private string ObtenerNombreMuelle(Embarque embarque)
        {
            ...
            if (embarque.OtrosMuelles && !string.IsNullOrEmpty(embarque.OtroMuelleNombre)) return embarque.OtroMuelleNombre;
        ...
        private List<CargaPorProductoExportadorDto> ExtraerCargas(LineUp lineup, int productoId, int? exportadorId)
        {
            if (lineup.Embarque.OtroMuelleCarga != null)
            {
                return lineup.Embarque.OtroMuelleCarga.OtroMuelleCargaDetalles.Where(d => d.MaterialPuerto.Id == productoId && (exportadorId == null || d.Exportador.Id == exportadorId.Value))
                    .GroupBy(d => new { d.Exportador, d.MaterialPuerto })
                    .Select(d => new CargaPorProductoExportadorDto
                    {
                        MaterialPuerto = _conversor.Convertir<MaterialPuerto, MaterialPuertoDto>(d.Key.MaterialPuerto),
                        Exportador = _conversor.Convertir<Exportador, ExportadorDto>(d.Key.Exportador),
                        Cantidad = d.Sum(c => c.CantidadTn)
                    }).ToList();
            }
            else if (lineup.ModuloDeCarga?.ModuloDeCargaPlanillaDeTurnos == null)
            {
            ...
            {
                return lineup.ModuloDeCarga.ModuloDeCargaPlanillaDeTurnos.SelectMany(pt => pt.ModuloDeCargaPlanillaDeTurnosDetallesLiquido)
                    .Where(d => d.MaterialPuerto.Id == productoId && (exportadorId == null || d.Exportador.Id == exportadorId.Value))
                    .GroupBy(d => new { d.Exportador, d.MaterialPuerto })
                    .Select(d => new CargaPorProductoExportadorDto
                    {
                        MaterialPuerto = _conversor.Convertir<MaterialPuerto, MaterialPuertoDto>(d.Key.MaterialPuerto),
                        Exportador = _conversor.Convertir<Exportador, ExportadorDto>(d.Key.Exportador),
                        Cantidad = d.Sum(c => c.Cantidad)
                    }).ToList();
            }
            else
            {
                return lineup.ModuloDeCarga.ModuloDeCargaPlanillaDeTurnos.SelectMany(pt => pt.ModuloDeCargaPlanillaDeTurnosDetallesSolido)
                    ...
                    .Select(d => new CargaPorProductoExportadorDto
                    {
                        MaterialPuerto = _conversor.Convertir<MaterialPuerto, MaterialPuertoDto>(d.Key.MaterialPuerto),
                        Exportador = _conversor.Convertir<Exportador, ExportadorDto>(d.Key.Exportador),
                        Cantidad = d.Sum(c => (decimal)c.Cantidad / 1000m)
                    }).ToList();
            }
        }

        private List<LineUp> ObtenerLineupsExcel(DateTime periodo, int? muelleId, int? embarqueId)
        {
            var nominacionesActivas = _repositorio.Incluir<Nominacion>().Where(n => n.FechaEliminacion == null);
            var embarquesValidosIds = nominacionesActivas
                .SelectMany(n => n.Embarques.Select(ne => ne.Embarque))
                .Union(nominacionesActivas.Select(n => n.Embarque))
                .Where(e => e != null)
            ...
            var lineups = _repositorio.Incluir<LineUp>().Where(l => embarquesValidosIds.Contains(l.Embarque.Id) && l.Embarque.Ubicacion == 1);
            // Filtro Periodo
            lineups = lineups.Where(l => l.ModuloDeCarga != null && (
                ...
                (
                    !l.ModuloDeCarga.ModuloDeCargaPeriodoDeCarga.Any() &&
                    l.Embarque.OtroMuelleCarga != null &&
                    l.Embarque.OtroMuelleCarga.OtroMuelleCargaDetalles.Any() &&
                    l.Embarque.OtroMuelleCarga.OtroMuelleCargaDetalles.Max(p => p.FechaHoraFin).Year == periodo.Year &&
                    l.Embarque.OtroMuelleCarga.OtroMuelleCargaDetalles.Max(p => p.FechaHoraFin).Month == periodo.Month
                ))
            );
            ...
            if (muelleId != null)
            {
                lineups = lineups.Where(l =>
                    // El Muelle será null en los casos anteriores a la implementación de Otros Muelles
                    l.Embarque.Muelle == null && (
                        (muelleId == 1 && l.Embarque.SanBenito) ||
                        (muelleId == 2 && l.Embarque.Vicentin) ||
                        (muelleId == 3 && l.Embarque.Noryon) ||
            ...
            if (embarqueId != null)
            {
                lineups = lineups.Where(l => l.Embarque.Id == embarqueId);
            }

            return lineups.GroupBy(l => l.Embarque.Id).Select(g => g.FirstOrDefault()).ToList();
        }
		...
		private DatosBaseProvisionInterno ObtenerDatosBaseProvision(int? muelleId, DateTime periodo, int? embarqueId, int? productoId, int? exportadorId, int? acuerdoId)
		{
			...
			var exportadorMOA = _repositorio.Obtener<Exportador>(idMOA);
