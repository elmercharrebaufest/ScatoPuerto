	...
	FiltrosAdministracionDto filtros = null)
		{
			...
			{
				var admEmbarque = _repositorio.Obtener<Embarque>(e => e.Id == item.IdEmbarque)?.AdministracionEmbarque;
				if (admEmbarque?.Estado != null)
				{
					var estadoDesc = admEmbarque.Estado.Descripcion;
		...
		{
			var estado = lineup.Embarque.AdministracionEmbarque?.Estado;
			if (estado != null)
			{
				var descripcion = estado.Descripcion;
		...
		public void EvaluarEstadoAplicadoParaEmbarque(int embarqueId, string usuario)
		{
			...
			if (embarque.AdministracionEmbarque?.Estado?.Id == estadoFacturado?.Id) return;
		...
		private void TransicionarAAplicado(Embarque embarque, string usuario)
		{
			...
			if (embarque.AdministracionEmbarque?.Estado?.Id == estadoAplicado.Id) return;

			if (embarque.AdministracionEmbarque == null)
			{
				var nuevaAdm = new AdministracionEmbarque
				{
					Estado = estadoAplicado
				};
				embarque.AdministracionEmbarque = nuevaAdm;
				_repositorio.Agregar(nuevaAdm);
			}
			else
			{
				embarque.AdministracionEmbarque.Estado = estadoAplicado;
			}
