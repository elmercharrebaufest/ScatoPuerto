...
{
    public class ProcesadorCrearTransportistaOrdenCargaFas : ProcesadorComando<CrearTransportistaOrdenCargaFas>
    {
        public ProcesadorCrearTransportistaOrdenCargaFas(IRepositorio repositorio, IConversor conversor, ILogger log)
            : base(repositorio, conversor, log)
        {
        }

        public override Resultado Ejecutar(CrearTransportistaOrdenCargaFas comando)
        {
            var resultado = new Resultado();
            var ordenCargaFas = Repositorio.Obtener<OrdenCargaFas>(comando.OrdenCargaFasId);
        ...
        private void Validar(CrearTransportistaOrdenCargaFas comando, Resultado resultado)
        {
            if (!Repositorio.Existe<OrdenCargaFas>(x => x.Id == comando.OrdenCargaFasId))
            {
