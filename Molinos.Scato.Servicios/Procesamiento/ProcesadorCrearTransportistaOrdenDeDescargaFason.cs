...
{
    public class ProcesadorCrearTransportistaOrdenDeDescargaFason : ProcesadorComando<CrearTransportistaOrdenDeDescargaFason>
    {
        public ProcesadorCrearTransportistaOrdenDeDescargaFason(IRepositorio repositorio, IConversor conversor, ILogger log)
            : base(repositorio, conversor, log)
        {
        }

        public override Resultado Ejecutar(CrearTransportistaOrdenDeDescargaFason comando)
        {
            var resultado = new Resultado();
            var OrdenDeDescargaFason = Repositorio.Obtener<OrdenDeDescargaFason>(comando.OrdenDeDescargaFasonId);
            ...
            if (!resultado.HayErrores)
            {
                try
                {
                    ...
                    OrdenDeDescargaFason.Transportista = transportista;
        ...
        private void Validar(CrearTransportistaOrdenDeDescargaFason comando, Resultado resultado)
        {
            if (!Repositorio.Existe<OrdenDeDescargaFason>(x => x.Id == comando.OrdenDeDescargaFasonId))
            {
