...
{
    public class ProcesadorActualizarDescargaUnidad : ProcesadorComando<ActualizarDescargaUnidad>
    {
        private ZSDWS_SCATO servicioSap;
        public ProcesadorActualizarDescargaUnidad(IRepositorio repositorio, IConversor conversor, ILogger log, ZSDWS_SCATO servicioSap)
            : base(repositorio, conversor, log)
        {
            this.servicioSap = servicioSap;
        }

        public override Resultado Ejecutar(ActualizarDescargaUnidad comando)
        {
            var resultado = new ResultadoCrear();
            var descarga = Repositorio.Obtener<DescargaUnidad>(x => (x.Id == comando.Dto.NroDescarga));
            try
            {
                ...
                if (descarga == null)
                {
                    ...
                    if (!resultado.HayErrores)
                    {
                        descarga = Conversor.Convertir<DescargaUnidadDto, DescargaUnidad>(comando.Dto);
                        descarga.FechaCierre = null;
                        descarga.Proveedor = proveedor;
                        descarga.DescargaUnidadItemPedidos = materialesPedidos.Select(x => new DescargaUnidadItemPedido()
                        {
                            ...
                            Werks = x.WERKS,
                            DescargaUnidad = descarga,
                            Material = materiales.FirstOrDefault(y => y.CodigoSAP == x.MATNR)
                        }).ToList();
                        Repositorio.Agregar(descarga);
                    }
                    
                }
                else
                {
                    descarga.Estado = comando.Dto.Estado;
                    descarga.Observaciones = comando.Dto.Observaciones;
                    descarga.FechaCierre = DateTime.Now;
                }
                if (!resultado.HayErrores)
                {
                    Repositorio.GuardarCambios();
                    resultado.Id = descarga.Id;
                }
            }
            catch (Exception e)
            {
        ...
        private void ConsultaPedidoResponse(ActualizarDescargaUnidad comando, ResultadoCrear resultado, out ZMMBALANZA2[] materialesPedidos, out List<string> materialsNr, out string codigoProveedor)
        {
            var respuesta =
