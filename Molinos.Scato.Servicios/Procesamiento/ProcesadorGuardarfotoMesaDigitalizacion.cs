        ...
        public override Resultado Ejecutar(GuardarfotoMesaDigitalizacion comando)
        {
            ...
            try
            {
                ...
                if (!comando.EsTemporal)
                {
                    ...
                    rutaDestino = comando.Directorio + (comando.Directorio.EndsWith("\\") ? "" : "\\") + subpath + "\\" + FotoCamionHelper.GenerarNombre(centroCodigoSAP, comando.NumeroDocumentoIngreso, comando.Patente, "Mesa", comando.Fecha, comando.TipoVehiculo) + ".jpeg";
                }
                else
                {
                    var subpath = "temp";
                    rutaDestino = comando.Directorio + (comando.Directorio.EndsWith("\\") ? "" : "\\") + subpath + "\\" + FotoCamionHelper.GenerarNombreTemporal(comando.NumeroDeTarjeta, comando.Fecha) + ".jpeg";
                }
