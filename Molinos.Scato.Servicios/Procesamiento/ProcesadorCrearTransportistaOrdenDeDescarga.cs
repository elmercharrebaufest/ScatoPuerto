...
{
    public class ProcesadorCrearTransportistaOrdenDeDescarga : ProcesadorComando<CrearTransportistaOrdenDeDescarga>
    {
        public ProcesadorCrearTransportistaOrdenDeDescarga(IRepositorio repositorio, IConversor conversor, ILogger log)
            : base(repositorio, conversor, log)
        {
        }

        public override Resultado Ejecutar(CrearTransportistaOrdenDeDescarga comando)
        {
            var resultado = new Resultado();
            var OrdenDeDescarga = Repositorio.Obtener<OrdenDeDescarga>(comando.OrdenDeDescargaId);
        ...
        private void Validar(CrearTransportistaOrdenDeDescarga comando, Resultado resultado)
        {
            if (!Repositorio.Existe<OrdenDeDescarga>(x => x.Id == comando.OrdenDeDescargaId))
            {
