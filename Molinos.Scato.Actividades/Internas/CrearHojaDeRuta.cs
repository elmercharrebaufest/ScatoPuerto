...
{
    public class CrearHojaDeRuta : CodeActivity<Resultado>
    {
        [RequiredArgument]
        public InArgument<HojaDeRutaDto> Orden { get; set; }
        ...
        public OutArgument<HojaDeRutaDto> HojaDeRuta { get; set; }

        public OutArgument<string> Numero { get; set; }
        ...
        {
            var orden = Orden.Get<HojaDeRutaDto>(context);
            ...
            try
            {
                ...
                var resultadoCrear = servicioComandos.Ejecutar(new Dominio.Comandos.CrearHojaDeRuta
                {
                    Orden = orden,
                ...
                var ordenDto = srvRepositorio.ObtenerHojaDeRuta(resultado.Id);
                if (ordenDto != null)
                {
                    HojaDeRuta.Set(context, ordenDto);
                    FechaInicio.Set(context, DateTime.Now);
                    Numero.Set(context, ordenDto.Numero);
                    TipoDocumentoIngreso.Set(context, Dominio.Enums.TipoDocumentoIngreso.HojaDeRuta);
                }
