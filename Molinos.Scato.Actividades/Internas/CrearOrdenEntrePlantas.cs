...
{
    public class CrearOrdenEntrePlantas : CodeActivity<Resultado>
    {
        [RequiredArgument]
        public InArgument<OrdenEntrePlantasDto> Orden { get; set; }
        ...
        public OutArgument<OrdenEntrePlantasDto> OrdenEntrePlantas { get; set; }

        public OutArgument<string> NumeroOrdenDeCarga { get; set; }
        ...
        {
            var orden = Orden.Get<OrdenEntrePlantasDto>(context);
            ...
            try
            {
                ...
                var resultadoCrear = servicioComandos.Ejecutar(new Dominio.Comandos.CrearOrdenEntrePlantas
                {
                    Orden = orden,
                ...
                var ordenDto = srvRepositorio.ObtenerOrdenEntrePlantas(resultado.Id);
                if (ordenDto != null)
                {
                    OrdenEntrePlantas.Set(context, ordenDto);
                    FechaInicio.Set(context, DateTime.Now);
                    NumeroOrdenDeCarga.Set(context, ordenDto.Numero);
                    TipoDocumentoIngreso.Set(context, Dominio.Enums.TipoDocumentoIngreso.OrdenEntrePlantas);
                }
