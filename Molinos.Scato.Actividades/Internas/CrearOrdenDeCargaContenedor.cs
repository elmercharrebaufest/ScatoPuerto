...
{
    public class CrearOrdenDeCargaContenedor : CodeActivity<Resultado>
    {
        [RequiredArgument]
        public InArgument<OrdenDeCargaContenedorDto> Orden { get; set; }
        ...
        public OutArgument<OrdenDeCargaContenedorDto> OrdenDeCargaContenedor { get; set; }

        public OutArgument<string> NroOrdenDeCargaContenedor { get; set; }
        ...
        {
            var orden = Orden.Get<OrdenDeCargaContenedorDto>(context);
            ...
            try
            {
                ...
                var resultadoCrear = servicioComandos.Ejecutar(new Dominio.Comandos.CrearOrdenDeCargaContenedor
                {
                    Orden = orden,
                ...
                var ordenDto = srvRepositorio.ObtenerOrdenDeCargaContenedor(resultado.Id);
                if (ordenDto != null)
                {
                    OrdenDeCargaContenedor.Set(context, ordenDto);
                    FechaInicio.Set(context, DateTime.Now);
                    NroOrdenDeCargaContenedor.Set(context, ordenDto.OrdenDeCargaContenedor);
                    TipoDocumentoIngreso.Set(context, Dominio.Enums.TipoDocumentoIngreso.OrdenDeCargaContenedor);
                }
