    ...
    BEGIN
        ...
        IF EXISTS (SELECT 1 FROM deleted) AND EXISTS (SELECT 1 FROM inserted) BEGIN
            ...
            BEGIN
                ...
                SELECT @idNominacion , d.id, 'Senasa', 'Exportador_Id', d.Exportador_Id, i.Exportador_Id , GETDATE(), NULL, NULL
            ...
            BEGIN
                INSERT INTO Auditoria
                SELECT @idNominacion , d.id, 'Senasa', 'TieneSenasa', d.TieneSenasa, i.TieneSenasa , GETDATE(), NULL, NULL
                FROM deleted AS d JOIN inserted AS i ON d.Id = i.Id
            ...
            BEGIN
                INSERT INTO Auditoria
                SELECT @idNominacion , d.id, 'Senasa', 'Consumo', d.Consumo, i.Consumo , GETDATE(), NULL, NULL
                FROM deleted AS d JOIN inserted AS i ON d.Id = i.Id
            ...
            IF((SELECT ACuentaDe FROM deleted) <> (SELECT ACuentaDe FROM inserted)) BEGIN
                INSERT INTO Auditoria
                SELECT @idNominacion , d.id, 'Senasa', 'ACuentaDe', d.ACuentaDe, i.ACuentaDe , GETDATE(), NULL, NULL
                FROM deleted AS d JOIN inserted AS i ON d.Id = i.Id
            ...
            IF ((SELECT Destino_Id FROM deleted) <> (SELECT Destino_Id FROM inserted)) BEGIN
                insert into Auditoria
                SELECT @idNominacion , d.id, 'Senasa', 'Destino_Id', d.Destino_Id, i.Destino_Id , GETDATE(), NULL, NULL
                FROM deleted AS d JOIN inserted AS i ON d.Id = i.Id
            ...
            IF ((SELECT [IP] FROM deleted) <> (SELECT [IP] FROM inserted)) BEGIN
                INSERT INTO Auditoria
                SELECT @idNominacion , d.id, 'Senasa', 'IP', d.[IP], i.[IP] , GETDATE(), NULL, NULL
                FROM deleted AS d JOIN inserted AS i ON d.Id = i.Id
            ...
            IF ((SELECT GMO FROM deleted) <> (SELECT GMO FROM inserted)) BEGIN
                INSERT INTO Auditoria
                SELECT @idNominacion , d.id, 'Senasa', 'GMO', d.GMO, i.GMO , GETDATE(), NULL, NULL
                FROM deleted AS d JOIN inserted AS i ON d.Id = i.Id
            ...
            IF ((SELECT FITO FROM deleted) <> (SELECT FITO FROM inserted)) BEGIN
                INSERT INTO Auditoria
                SELECT @idNominacion , d.id, 'Senasa', 'FITO', d.FITO, i.FITO , GETDATE(), NULL, NULL
                FROM deleted AS d JOIN inserted AS i ON d.Id = i.Id
            ...
            IF ((SELECT MuestraOficial FROM deleted) <> (SELECT MuestraOficial FROM inserted)) BEGIN
                INSERT INTO Auditoria
                SELECT @idNominacion , d.id, 'Senasa', 'MuestraOficial', d.MuestraOficial, i.MuestraOficial , GETDATE(), NULL, NULL
                FROM deleted AS d JOIN inserted AS i ON d.Id = i.Id
            ...
            IF ((SELECT CertificadoInocuidad FROM deleted) <> (SELECT CertificadoInocuidad FROM inserted)) BEGIN
                INSERT INTO Auditoria
                SELECT @idNominacion , d.id, 'Senasa', 'CertificadoInocuidad', d.CertificadoInocuidad, i.CertificadoInocuidad , GETDATE(), NULL, NULL
                FROM deleted AS d JOIN inserted AS i ON d.Id = i.Id
            ...
            IF ((SELECT CertificadoVeterinario FROM deleted) <> (SELECT CertificadoVeterinario from inserted)) BEGIN
                INSERT INTO Auditoria
                SELECT @idNominacion , d.id, 'Senasa', 'CertificadoVeterinario', d.CertificadoVeterinario, i.CertificadoVeterinario , GETDATE(), NULL, NULL
                FROM deleted AS d JOIN inserted AS i ON d.Id = i.Id
            ...
            IF ((SELECT Observaciones FROM deleted) <> (SELECT Observaciones FROM inserted)) BEGIN
                INSERT INTO Auditoria
                SELECT @idNominacion , d.id, 'Senasa', 'Observaciones', d.Observaciones, i.Observaciones , GETDATE(), NULL, NULL
                FROM deleted AS d JOIN inserted AS i ON d.Id = i.Id
        ...
        ELSE IF EXISTS (SELECT 1 FROM inserted) AND (@dateDiff > 5) BEGIN
            INSERT INTO Auditoria
            SELECT @idNominacion , id, 'Senasa', 'Exportador', NULL, @exportadorNuevo, GETDATE(), NULL, NULL
            FROM inserted
        ...
        ELSE IF EXISTS (SELECT 1 FROM deleted) BEGIN
            INSERT INTO Auditoria
            SELECT @idNominacion, Id, 'Senasa', 'Exportador', @exportadorPrevio, NULL, GETDATE(), NULL, NULL
            FROM deleted
